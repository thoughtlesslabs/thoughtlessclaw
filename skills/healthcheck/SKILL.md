---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for Skynet deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, Skynet cron scheduling for periodic checks, or version status checks on a machine running Skynet (laptop, workstation, Pi, VPS).
---

# Skynet Host Hardening

## Overview

Assess and harden the host running Skynet, then align it to a user-defined risk tolerance without breaking access. Use Skynet security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim Skynet changes the host firewall, SSH, or OS updates; it does not.
...
5. Skynet gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if Skynet can access sensitive data.
...
### 2) Run Skynet security audits (read-only)

As part of the default read-only checks, run `skynet security audit --deep`. Only offer alternatives if the user requests them:

1. `skynet security audit` (faster, non-probing)
2. `skynet security audit --json` (structured output)

Offer to apply Skynet safe defaults (numbered):

1. `skynet security audit --fix`

Be explicit that `--fix` only tightens Skynet defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check Skynet version/update status (read-only)

As part of the default read-only checks, run `skynet update status`.
...
- Credential hygiene notes (location of Skynet creds, prefer disk encryption)
...
After Skynet install or first hardening pass, run at least one baseline audit and version check:

- `skynet security audit`
- `skynet security audit --deep`
- `skynet update status`

Ongoing monitoring is recommended. Use the Skynet cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `skynet cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `skynet update status`
...
Before creating, `skynet cron list` and match on exact `name`. If found, `skynet cron edit <id> ...`.
If not found, `skynet cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `skynet update status` (preferred for source checkouts and channels)
2. `npm view skynet version` (published npm version)

## Skynet command accuracy

Use only supported commands and flags:

- `skynet security audit [--deep] [--fix] [--json]`
- `skynet status` / `skynet status --deep`
- `skynet health --json`
- `skynet update status`
- `skynet cron add|list|runs|run`

Do not invent CLI flags or imply Skynet enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by Skynet compaction:
