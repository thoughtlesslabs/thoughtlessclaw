...
- `/usage cost` in chats: local cost summary aggregated from Skynet session logs.
- CLI: `skynet status --usage` prints a full per-provider breakdown.
- CLI: `skynet channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
