---
summary: "Use MiniMax M2.1 in Skynet"
read_when:
  - You want MiniMax models in Skynet
...
```bash
skynet plugins enable minimax-portal-auth  # skip if already loaded.
skynet gateway restart  # restart if gateway is already running
skynet onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/skynet/skynet/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `skynet.json`:
...
- Use `skynet models list` and `skynet models set minimax/MiniMax-M2.1` to switch.
...
- Running `skynet configure` and selecting **MiniMax M2.1**, or
