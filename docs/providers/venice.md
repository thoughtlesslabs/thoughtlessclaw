---
summary: "Use Venice AI privacy-focused models in Skynet"
read_when:
  - You want privacy-focused inference in Skynet
...
```bash
skynet onboard --auth-choice venice-api-key
```
...
```bash
skynet agent --model venice/llama-3.3-70b --message "Hello, are you working?"
```

## Model Selection

After setup, Skynet shows all available Venice models. Pick based on your needs:
...
```bash
skynet models set venice/claude-opus-45
skynet models set venice/llama-3.3-70b
```

List all available models:

```bash
skynet models list | grep venice
```

## Configure via `skynet configure`

1. Run `skynet configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
Skynet automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
skynet agent --model venice/llama-3.3-70b --message "Quick health check"

# Use Claude via Venice (anonymized)
skynet agent --model venice/claude-opus-45 --message "Summarize this task"

# Use uncensored model
skynet agent --model venice/venice-uncensored --message "Draft options"

# Use vision model with image
skynet agent --model venice/qwen3-vl-235b-a22b --message "Review attached image"

# Use coding model
skynet agent --model venice/qwen3-coder-480b-a35b-instruct --message "Refactor this function"
```
...
The Venice model catalog updates dynamically. Run `skynet models list` to see currently available models. Some models may be temporarily offline.
