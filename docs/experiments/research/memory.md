...
read_when:
  - Designing workspace memory (~/.skynet/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep Skynet integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.skynet/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.skynet/workspace` as canonical human-readable memory.
...
```
~/.skynet/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in Skynet**, but keep a separable core library.

### Why integrate into Skynet?

- Skynet already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `skynet memory recall "…" --k 25 --since 30d`
  - `skynet memory reflect --since 7d`
...
Pragmatic take for `~/.skynet/workspace`:
