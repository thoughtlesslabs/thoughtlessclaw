...
description: "Complete field-by-field reference for ~/.skynet/skynet.json"
summary: "Complete reference for every Skynet config key, defaults, and channel settings"
read_when:
...
Every field available in `~/.skynet/skynet.json`. For a task-oriented overview, see [Configuration](/gateway/configuration).

Config format is **JSON5** (comments + trailing commas allowed). All fields are optional — Skynet uses safe defaults when omitted.
...
- Legacy single-account Baileys auth dir is migrated by `skynet doctor` into `whatsapp/default`.
...
```json5
{
  channels: {
    discord: {
      ...
      dm: { enabled: true, groupEnabled: false, groupChannels: ["skynet-dm"] },
      guilds: {
        "123456789012345678": {
...
Mattermost ships as a plugin: `skynet plugins install @skynet/mattermost`.
...
Skynet spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
```json5
{
  ...
  agents: {
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["reisponde", "@skynet"] },
      },
    ],
  },
}
...
- `config: true` enables `/config` (reads/writes `skynet.json`).
...
Optional repository root shown in the system prompt's Runtime line. If unset, Skynet auto-detects by walking upward from the workspace.
...
- `model.primary`: format `provider/model` (e.g. `anthropic/claude-opus-4-6`). If you omit the provider, Skynet assumes `anthropic` (deprecated).
...
- `none`: per-scope sandbox workspace under `~/.skynet/sandboxes`
...
noVNC observer access uses VNC auth by default and Skynet emits a short-lived token URL (instead of exposing the password in the shared URL).

- `allowHostControl: false` (default) blocks sandboxed sessions from targeting the host browser.
- `network` defaults to `skynet-sandbox-browser` (dedicated bridge network). Set to `bridge` only when you explicitly want global bridge connectivity.
    ...
    list: [
      {
        ...
        agentDir: "~/.skynet/agents/main/agent",
        ...
        groupChat: { mentionPatterns: ["@skynet"] },
        sandbox: { mode: "off" },
...
| `group:nodes`      | `nodes`                                                                                  |
| `group:skynet`   | All built-in tools (excludes provider plugins)                                           |
...
Skynet uses the pi-coding-agent model catalog. Add custom providers via `models.providers` in config or `~/.skynet/agents/<agentId>/agent/models.json`.
...
- Override agent config root with `SKYNET_AGENT_DIR` (or `PI_CODING_AGENT_DIR`).
...
Set `OPENCODE_API_KEY` (or `OPENCODE_ZEN_API_KEY`). Shortcut: `skynet onboard --auth-choice opencode-zen`.
...
Set `ZAI_API_KEY`. `z.ai/*` and `z-ai/*` are accepted aliases. Shortcut: `skynet onboard --auth-choice zai-api-key`.
...
For the China endpoint: `baseUrl: "https://api.moonshot.cn/v1"` or `skynet onboard --auth-choice moonshot-api-key-cn`.
...
Anthropic-compatible, built-in provider. Shortcut: `skynet onboard --auth-choice kimi-code-api-key`.
...
Base URL should omit `/v1` (Anthropic client appends it). Shortcut: `skynet onboard --auth-choice synthetic-api-key`.
...
Set `MINIMAX_API_KEY`. Shortcut: `skynet onboard --auth-choice minimax-api`.
...
- Loaded from `~/.skynet/extensions`, `<workspace>/.skynet/extensions`, plus `plugins.load.paths`.
...
```json5
{
  gateway: {
    ...
    auth: {
      ...
      token: "your-token",
      // password: "your-password", // or SKYNET_GATEWAY_PASSWORD
...
- `port`: single multiplexed port for WS + HTTP. Precedence: `--port` > `SKYNET_GATEWAY_PORT` > `gateway.port` > `18789`.
...
Convenience flags: `--dev` (uses `~/.skynet-dev` + port `19001`), `--profile <name>` (uses `~/.skynet-<name>`).
...
```json5
{
  hooks: {
    ...
    presets: ["gmail"],
    transformsDir: "~/.skynet/hooks/transforms",
    mappings: [
      {
        match: { path: "gmail" },
        action: "agent",
...
Auth: `Authorization: Bearer <token>` or `x-skynet-token: <token>`.
...
- Gateway auto-starts `gog gmail watch serve` on boot when configured. Set `SKYNET_SKIP_GMAIL_WATCHER=1` to disable.
...
```json5
{
  canvasHost: {
    ...
    liveReload: true,
    // enabled: false, // or SKYNET_SKIP_CANVAS_HOST=1
  },
}
```

- Serves agent-editable HTML/CSS/JS and A2UI over HTTP under the Gateway port:
  - `http://<gateway-host>:<gateway.port>/__skynet__/canvas/`
  - `http://<gateway-host>:<gateway.port>/__skynet__/a2ui/`
...
- Also serves A2UI at `/__skynet__/a2ui/`.
...
- Hostname defaults to `skynet`. Override with `SKYNET_MDNS_HOSTNAME`.
...
Writes a unicast DNS-SD zone under `~/.skynet/dns/`. For cross-network discovery, pair with a DNS server (CoreDNS recommended) + Tailscale split DNS.

Setup: `skynet dns setup --apply`.
...
- `.env` files: CWD `.env` + `~/.skynet/.env` (neither overrides existing vars).
...
{
  gateway: {
    auth: { token: "${SKYNET_GATEWAY_TOKEN}" },
  },
}
```
...
- Legacy OAuth imports from `~/.skynet/credentials/oauth.json`.
...
- Default log file: `/tmp/skynet/skynet-YYYY-MM-DD.log`.
...
Current builds no longer include the TCP bridge. Nodes connect over the Gateway WebSocket. `bridge.*` keys are no longer part of the config schema (validation fails until removed; `skynet doctor --fix` can strip unknown keys).
