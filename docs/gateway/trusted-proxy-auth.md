...
read_when:
  - Running Skynet behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of Skynet
...
- You run Skynet behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. Skynet checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. Skynet extracts the user identity from the configured header
...
- Skynet can stay on loopback HTTP behind the proxy.
...
If Skynet itself serves HTTPS directly (no TLS-terminating proxy), set:
...
```yaml
routes:
  - from: https://skynet.example.com
    to: http://skynet-gateway:18789
    policy:
      - allow:
          or:
            - email:
...
`skynet security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
3. Update Skynet config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `skynet security audit` and review findings
