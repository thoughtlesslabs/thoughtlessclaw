...
- During heartbeat runs, Skynet treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `skynet sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, Skynet skips the heartbeat run entirely (no model call).
...
```bash
skynet system event --text "Check for urgent follow-ups" --mode now
```
