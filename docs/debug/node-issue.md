...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in Skynet
...
Running Skynet via Node with `tsx` fails at startup with:

```
[skynet] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `skynet status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch skynet.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node skynet.mjs status` works on Node 25.
