...
```bash
skynet tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `skynet tui --deliver`
...
- Check the Gateway logs: `skynet logs --follow`.
- Confirm the agent can run: `skynet status` and `skynet models status`.
...
- No agents in picker: check `skynet agents list` and your routing config.
