...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/skynet`)
...
If the page fails to load, start the Gateway first: `skynet gateway`.
...
you revoke it with `skynet devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.skynet/skynet.json` (`config.get`, `config.set`)
...
- Stop:
  ...
  - Type `/stop` (or standalone abort phrases like `stop`, `stop action`, `stop run`, `stop skynet`, `please stop`) to abort out-of-band
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. Skynet
...
Skynet **blocks** Control UI connections without device identity.
