...
- `~/Library/Application Support/Skynet/canvas/<session>/...`

Canvas 面板通过**自定义 URL 方案**提供这些文件：

- `skynet-canvas://<session>/<path>`

示例：

- `skynet-canvas://main/` → `<canvasRoot>/main/index.html`
- `skynet-canvas://main/assets/app.css` → `<canvasRoot>/main/assets/app.css`
- `skynet-canvas://main/widgets/todo/` → `<canvasRoot>/main/widgets/todo/index.html`
...
```bash
skynet nodes canvas present --node <id>
skynet nodes canvas navigate --node <id> --url "/"
skynet nodes canvas eval --node <id> --js "document.title"
skynet nodes canvas snapshot --node <id>
```
...
```
http://<gateway-host>:18793/__skynet__/a2ui/
```
...
skynet nodes canvas a2ui push --jsonl /tmp/a2ui-v0.8.jsonl --node <id>
```

快速测试：

```bash
skynet nodes canvas a2ui push --node <id> --text "Hello from A2UI"
```
...
```js
window.location.href = "skynet://agent?message=Review%20this%20design";
```
