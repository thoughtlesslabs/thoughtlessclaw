...
你可以在 `~/.skynet/skynet.json` 中覆盖此设置：
...
所有日志配置都在 `~/.skynet/skynet.json` 的 `logging` 下。
...
- Skynet 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
- 你也可以使用 `skynet plugins enable diagnostics-otel` 启用插件。
...
- `skynet.tokens`（计数器，属性：`skynet.token`、`skynet.channel`、`skynet.provider`、`skynet.model`）
- `skynet.cost.usd`（计数器，属性：`skynet.channel`、`skynet.provider`、`skynet.model`）
- `skynet.run.duration_ms`（直方图，属性：`skynet.channel`、`skynet.provider`、`skynet.model`）
- `skynet.context.tokens`（直方图，属性：`skynet.context`、`skynet.channel`、`skynet.provider`、`skynet.model`）

消息流：

- `skynet.webhook.received`（计数器，属性：`skynet.channel`、`skynet.webhook`）
- `skynet.webhook.error`（计数器，属性：`skynet.channel`、`skynet.webhook`）
- `skynet.webhook.duration_ms`（直方图，属性：`skynet.channel`、`skynet.webhook`）
- `skynet.message.queued`（计数器，属性：`skynet.channel`、`skynet.source`）
- `skynet.message.processed`（计数器，属性：`skynet.channel`、`skynet.outcome`）
- `skynet.message.duration_ms`（直方图，属性：`skynet.channel`、`skynet.outcome`）

队列 + 会话：

- `skynet.queue.lane.enqueue`（计数器，属性：`skynet.lane`）
- `skynet.queue.lane.dequeue`（计数器，属性：`skynet.lane`）
- `skynet.queue.depth`（直方图，属性：`skynet.lane` 或 `skynet.channel=heartbeat`）
- `skynet.queue.wait_ms`（直方图，属性：`skynet.lane`）
- `skynet.session.state`（计数器，属性：`skynet.state`、`skynet.reason`）
- `skynet.session.stuck`（计数器，属性：`skynet.state`）
- `skynet.session.stuck_age_ms`（直方图，属性：`skynet.state`）
- `skynet.run.attempt`（计数器，属性：`skynet.attempt`）
...
- `skynet.webhook.error`
  - `skynet.channel`、`skynet.webhook`、`skynet.chatId`、`skynet.error`
- `skynet.message.processed`
  - `skynet.channel`、`skynet.outcome`、`skynet.chatId`、`skynet.messageId`、`skynet.sessionKey`、`skynet.sessionId`、`skynet.reason`
- `skynet.session.stuck`
  - `skynet.state`、`skynet.ageMs`、`skynet.queueDepth`、`skynet.sessionKey`、`skynet.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `skynet doctor`。
