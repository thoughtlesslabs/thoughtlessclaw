...
Skynet integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- Skynet owns lifecycle when `channels.signal.autoStart=true`.
...
- Skynet spawns `imsg rpc` as a child process (legacy iMessage integration).
