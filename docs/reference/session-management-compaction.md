...
This document explains how Skynet manages sessions end-to-end:
...
Skynet is designed around a single **Gateway process** that owns session state.
...
Skynet persists sessions in two layers:
...
- Transcripts: `~/.skynet/agents/<agentId>/sessions/<sessionId>.jsonl`
  - Telegram topic sessions: `.../<sessionId>-topic-<threadId>.jsonl`

Skynet resolves these via `src/config/sessions.ts`.
...
In `mode: "warn"`, Skynet reports potential evictions but does not mutate the store/files.
...
- `cron.runLog.maxBytes` + `cron.runLog.keepLines` prune `~/.skynet/cron/runs/<jobId>.jsonl` files (defaults: `2_000_000` bytes and `2000` lines).
...
Skynet intentionally does **not** “fix up” transcripts; the Gateway uses `SessionManager` to read/write them.
...
These are Pi runtime semantics (Skynet consumes the events, but Pi decides when to compact).
...
Skynet also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, Skynet bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it’s already higher, Skynet leaves it alone.
...
Skynet supports “silent” turns for background tasks where the user should not see intermediate output.

Convention:

- The assistant starts its output with `NO_REPLY` to indicate “do not deliver a reply to the user”.
- Skynet strips/suppresses this in the delivery layer.

As of `2026.1.10`, Skynet also suppresses **draft/typing streaming** when a partial chunk begins with `NO_REPLY`, so silent operations don’t leak partial output mid-turn.
...
Skynet uses the **pre-threshold flush** approach:
...
Pi also exposes a `session_before_compact` hook in the extension API, but Skynet’s
...
- Store vs transcript mismatch? Confirm the Gateway host and the store path from `skynet status`.
