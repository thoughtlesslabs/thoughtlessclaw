...
- With Anthropic API-key auth profiles, Skynet seeds `cacheRetention: "short"` for Anthropic model refs when unset.
...
For `openrouter/anthropic/*` model refs, Skynet injects Anthropic `cache_control` on system/developer prompt blocks to improve prompt-cache reuse.
...
Skynet exposes dedicated cache-trace diagnostics for embedded agent runs.
...
diagnostics:
  cacheTrace:
    enabled: true
    filePath: "~/.skynet/logs/cache-trace.jsonl" # optional
...
- `filePath`: `$SKYNET_STATE_DIR/logs/cache-trace.jsonl`
...
- `SKYNET_CACHE_TRACE=1` enables cache tracing.
- `SKYNET_CACHE_TRACE_FILE=/path/to/cache-trace.jsonl` overrides output path.
- `SKYNET_CACHE_TRACE_MESSAGES=0|1` toggles full message payload capture.
- `SKYNET_CACHE_TRACE_PROMPT=0|1` toggles prompt text capture.
- `SKYNET_CACHE_TRACE_SYSTEM=0|1` toggles system prompt capture.
