...
read_when:
  - You want to place an outbound voice call from Skynet
...
Voice calls for Skynet via a plugin. Supports outbound notifications and
...
- Use `skynet voicecall ...` or the `voice_call` tool
...
```bash
skynet plugins install ./extensions/voice-call
