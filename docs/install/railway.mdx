...
Deploy Skynet on Railway with a one-click template and finish setup in your browser.
...
- `https://<your-railway-domain>/skynet` — Control UI
...
- `SKYNET_STATE_DIR=/data/.skynet` (recommended)
- `SKYNET_WORKSPACE_DIR=/data/workspace` (recommended)
- `SKYNET_GATEWAY_TOKEN` (recommended; treat as an admin secret)
...
This exports your Skynet state + workspace so you can migrate to another host without losing config or memory.
