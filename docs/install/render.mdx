...
Deploy Skynet on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/skynet/skynet)
...
repository configures everything needed to run Skynet:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: SKYNET_STATE_DIR
        value: /data/.skynet
      - key: SKYNET_WORKSPACE_DIR
        value: /data/workspace
      - key: SKYNET_GATEWAY_TOKEN
...
The web dashboard is available at `https://<your-service>.onrender.com/skynet`.
...
If you use the original Skynet repository, Render will not auto-deploy your Skynet. To update it, run a manual Blueprint sync from the dashboard.
...
For Skynet, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any Skynet host.
