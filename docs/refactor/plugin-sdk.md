...
- Publish as `skynet/plugin-sdk` (or export from core under `skynet/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `SkynetPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: SkynetConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: SkynetConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: SkynetConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: SkynetConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: SkynetConfig): string;
  };
};
```
...
- Introduce `skynet/plugin-sdk`.
- Add `api.runtime` to `SkynetPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `skynetRuntime: ">=2026.2.0"`).
