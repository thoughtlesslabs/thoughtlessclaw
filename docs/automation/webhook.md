...
read_when:
  ...
  - Wiring external systems into Skynet
title: "Webhooks"
---
...
- `hooks.transformsDir` + `transform.module` loads a JS/TS module for custom logic.
  - `hooks.transformsDir` (if set) must stay within the transforms root under your Skynet config directory (typically `~/.skynet/hooks/transforms`).
...
- `skynet webhooks gmail setup` writes `hooks.gmail` config for `skynet webhooks gmail run`.
