...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `skynet sandbox explain`.
...
Legacy `agent.*` configs are migrated by `skynet doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=skynet-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${SKYNET_STATE_DIR:-$HOME/.skynet}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
