---
summary: "Direct `skynet agent` CLI runs (with optional delivery)"
read_when:
  - Adding or modifying the agent CLI entrypoint
title: "Agent Send"
---

# `skynet agent` (direct agent runs)

`skynet agent` runs a single agent turn without needing an inbound chat message.
...
- Optional delivery back to a channel with `--deliver` + `--channel` (target formats match `skynet message --target`).
...
```bash
skynet agent --to +15555550123 --message "status update"
skynet agent --agent ops --message "Summarize logs"
...
skynet agent --to +15555550123 --message "Summon reply" --deliver
