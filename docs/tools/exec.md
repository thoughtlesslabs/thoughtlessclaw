...
- `gateway`/`node` approvals are controlled by `~/.skynet/exec-approvals.json`.
...
- `host=sandbox`: runs `sh -lc` (login shell) inside the container, so `/etc/profile` may reset `PATH`.
  Skynet prepends `env.PATH` after profile sourcing via an internal env var (no shell interpolation);
...
`skynet security audit` warns when interpreter/runtime `safeBins` entries are missing explicit profiles, and `skynet doctor --fix` can scaffold missing custom `safeBinProfiles` entries.
