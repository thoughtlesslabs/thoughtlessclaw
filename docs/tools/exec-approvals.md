...
- **gateway host** → `skynet` process on the gateway machine
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.skynet/exec-approvals.sock",
...
- allowlist entries live in host-local `~/.skynet/exec-approvals.json` under `agents.<id>.allowlist` (or via Control UI / `skynet approvals allowlist ...`).
- `skynet security audit` warns with `tools.exec.safe_bins_interpreter_unprofiled` when interpreter/runtime bins appear in `safeBins` without explicit profiles.
- `skynet doctor --fix` can scaffold missing custom `safeBinProfiles.<bin>` entries as `{}` (review and tighten afterward). Interpreter/runtime bins are not auto-scaffolded.
...
`~/.skynet/exec-approvals.json` directly.

CLI: `skynet approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
