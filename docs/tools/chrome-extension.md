---
summary: "Chrome extension: let Skynet drive your existing Chrome tab"
read_when:
...
The Skynet Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate skynet-managed Chrome profile.
...
Skynet then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the Skynet release (npm package) as static files. There is no separate “build” step.

After upgrading Skynet:

- Re-run `skynet browser extension install` to refresh the installed files under your Skynet state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (set gateway token once)

Skynet ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.

Before first attach, open extension Options and set:

- `Port` (default `18792`)
- `Gateway token` (must match `gateway.auth.token` / `SKYNET_GATEWAY_TOKEN`)
...
- Open the tab you want Skynet to control.
...
- `ON`: attached; Skynet can drive that tab.
...
Debugging: `skynet sandbox explain`
...
`skynet browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `skynet browser extension install` first to copy the extension to a stable location under your Skynet state directory.
...
- **This is not isolated** like the dedicated skynet-managed profile.
