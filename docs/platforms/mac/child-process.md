...
service via the external `skynet` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `bot.molt.gateway`
  (or `bot.molt.<profile>` when using `--profile`/`SKYNET_PROFILE`; legacy `com.skynet.*` is supported).
