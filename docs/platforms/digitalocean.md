---
summary: "Skynet on DigitalOcean (simple paid VPS option)"
read_when:
  - Setting up Skynet on DigitalOcean
  - Looking for cheap VPS hosting for Skynet
title: "DigitalOcean"
---

# Skynet on DigitalOcean

## Goal

Run a persistent Skynet Gateway on DigitalOcean for **$6/month** (or $4/mo with reserved pricing).
...
# Check service
systemctl --user status skynet-gateway.service

# View logs
journalctl --user -u skynet-gateway.service -f
```
...
```bash
skynet channels login whatsapp
# Scan QR code
```
...
- `~/.skynet/workspace/` — workspace (SOUL.md, memory, etc.)
...
journalctl -u skynet --no-pager -n 50
```
