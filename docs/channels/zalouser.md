...
read_when:
  - Setting up Zalo Personal for Skynet
...
- Install via CLI: `skynet plugins install @skynet/zalouser`
- Or from a source checkout: `skynet plugins install ./extensions/zalouser`
...
- `skynet pairing list zalouser`
- `skynet pairing approve zalouser <code>`
...
- On startup, Skynet resolves group/user names in allowlists to IDs and logs the mapping; unresolved entries are kept as typed.
...
- Re-login: `skynet channels logout --channel zalouser && skynet channels login --channel zalouser`
