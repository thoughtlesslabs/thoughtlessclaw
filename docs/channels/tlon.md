...
Tlon is a decentralized messenger built on Urbit. Skynet connects to your Urbit ship and can
...
```bash
skynet plugins install @skynet/tlon
```

Local checkout (when running from a git repo):

```bash
skynet plugins install ./extensions/tlon
```
...
By default, Skynet blocks private/internal hostnames and IP ranges for this plugin (SSRF hardening).
...
Use these with `skynet message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, Skynet replies in-thread.
