...
Matrix is an open, decentralized messaging protocol. Skynet connects as a Matrix **user**
...
```bash
skynet plugins install @skynet/matrix
```

Local checkout (when running from a git repo):

```bash
skynet plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `skynet plugins install @skynet/matrix`
   - From a local checkout: `skynet plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: Skynet calls the same
     login endpoint, stores the access token in `~/.skynet/credentials/matrix/credentials.json`,
...
- On first connection, Skynet requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  Skynet logs a warning.
...
`~/.skynet/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `skynet pairing list matrix`
  - `skynet pairing approve matrix <CODE>`
...
- On startup, Skynet resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
