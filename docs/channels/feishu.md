...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects Skynet to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.
...
```bash
skynet plugins install @skynet/feishu
```

Local checkout (when running from a git repo):

```bash
skynet plugins install ./extensions/feishu
```
...
If you just installed Skynet, run the wizard:
...
2. Configuring app credentials in Skynet
...
1. You already ran `skynet channels add` for Feishu
2. The gateway is running (`skynet gateway status`)
...
```bash
skynet pairing approve feishu <CODE>
```
...
2. Run `skynet logs --follow` and look for `chat_id`
...
2. Run `skynet logs --follow` and look for `open_id`
...
| Command                    | Description                   |
| -------------------------- | ----------------------------- |
| `skynet gateway status`  | Show gateway status           |
| `skynet gateway install` | Install/start gateway service |
| `skynet gateway stop`    | Stop gateway service          |
| `skynet gateway restart` | Restart gateway service       |
| `skynet logs --follow`   | Tail gateway logs             |
...
4. Check logs: `skynet logs --follow`
...
5. Ensure the gateway is running: `skynet gateway status`
6. Check logs: `skynet logs --follow`
  ...
  agents: {
    list: [
      { id: "main" },
      {
        ...
        agentDir: "/home/user/.skynet/agents/clawd-fan/agent",
      },
      {
        id: "clawd-xi",
        workspace: "/home/user/clawd-xi",
        agentDir: "/home/user/.skynet/agents/clawd-xi/agent",
      },
    ],
  },
  bindings: [
