---
summary: "Mattermost bot setup and Skynet config"
read_when:
...
```bash
skynet plugins install @skynet/mattermost
```

Local checkout (when running from a git repo):

```bash
skynet plugins install ./extensions/mattermost
```
...
4. Configure Skynet and start the gateway.
...
- Approve via:
  - `skynet pairing list mattermost`
  - `skynet pairing approve mattermost <CODE>`
...
Use these target formats with `skynet message send` or cron/webhooks:
