...
read_when:
  - Setting up Twitch chat integration for Skynet
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. Skynet connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
skynet plugins install @skynet/twitch
```

Local checkout (when running from a git repo):

```bash
skynet plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `SKYNET_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "skynet", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use SKYNET_TWITCH_ACCESS_TOKEN env var)
...
```bash
SKYNET_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
