...
```bash
skynet plugins install @skynet/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
skynet plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "Skynet" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `skynet pairing list nextcloud-talk`
  - `skynet pairing approve nextcloud-talk <CODE>`
