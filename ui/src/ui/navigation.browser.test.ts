...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/skynet/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/skynet");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/skynet/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__SKYNET_CONTROL_UI_BASE_PATH__ = "/skynet";
    const app = mountApp("/skynet/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/skynet");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/skynet/sessions");
  });
