...
function resolveSkynetRoot(): string {
  ...
  const override = process.env.SKYNET_ROOT?.trim();
  if (override) {
  ...
  throw new Error("Unable to resolve core root. Set SKYNET_ROOT to the package root.");
}
...
async function importCoreExtensionAPI(): Promise<{
  ...
  const distPath = path.join(resolveSkynetRoot(), "dist", "extensionAPI.js");
