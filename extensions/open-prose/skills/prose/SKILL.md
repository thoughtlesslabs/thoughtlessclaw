...
metadata: { "skynet": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == Skynet `sessions_spawn`
- **File I/O** == Skynet `read`/`write`
- **Remote fetch** == Skynet `web_fetch` (or `exec` with curl when POST is required)
