...
import type { SkynetPluginApi, SkynetPluginService } from "skynet/plugin-sdk";
...
function normalizeDenyList(cfg: SkynetPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: SkynetPluginApi["config"]): string[] {
...
export default function register(api: SkynetPluginApi) {
  ...
  const timerService: SkynetPluginService = {
