...
Skynet extension for Zalo Personal Account messaging via [zca-cli](https://zca-cli.dev).
...
```bash
skynet channels login --channel zalouser              # Login via QR
skynet channels login --channel zalouser --account work
...
```bash
skynet directory self --channel zalouser
skynet directory peers list --channel zalouser --query "name"
skynet directory groups list --channel zalouser --query "work"
skynet directory groups members --channel zalouser --group-id <id>
```
...
```bash
# Text
skynet message send --channel zalouser --target <threadId> --message "message"

# Media (URL)
skynet message send --channel zalouser --target <threadId> --message "caption" --media-url "https://example.com/img.jpg"
```

### Listener

The listener runs inside the Gateway when the channel is enabled. For debugging,
use `skynet channels logs --channel zalouser` or run `zca listen` directly.
...
```bash
skynet channels login --channel zalouser --account work
skynet message send --channel zalouser --account work --target <id> --message "Hello"
