...
`skynet.invoke --each`) without adding new Skynet code per workflow.
...
This extension depends on Skynet internal modules (the embedded agent runner).
It is intended to ship as a **bundled** Skynet extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.skynet/extensions` as a standalone plugin directory.
