...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["skynetPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["skynet", "pollId"]) ??
    readNestedString(value, ["skynet", "poll", "id"]) ??
    readNestedString(value, ["data", "skynetPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "skynet", "pollId"]);
  if (!pollId) {
    return null;
  }
    ...
    actions: [
      {
        ...
        msteams: {
          type: "messageBack",
          text: "skynet poll vote",
          displayText: "Vote recorded",
          value: { skynetPollId: pollId, pollId },
        },
      },
    ],
  };
