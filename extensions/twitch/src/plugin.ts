/**
 * Twitch channel plugin for Skynet.
 *
...
import { buildChannelConfigSchema } from "skynet/plugin-sdk";
 ...
 * for Skynet. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<TwitchAccountConfig> = {
  ...
  config: {
    ...
    listAccountIds: (cfg: SkynetConfig): string[] => listAccountIds(cfg),

    /** Resolve an account config by ID */
    resolveAccount: (cfg: SkynetConfig, accountId?: string | null): TwitchAccountConfig => {
    ...
    isConfigured: (_account: unknown, cfg: SkynetConfig): boolean => {
