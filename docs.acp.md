...
This document describes how the Skynet ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`skynet acp` exposes an ACP agent over stdio and forwards prompts to a running
Skynet Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
- ACP client spawns `skynet acp` and speaks ACP messages over stdio.
...
`skynet acp` resolves the Gateway URL and auth from CLI flags or config:
