...
export function resolveConfigDir(
  ...
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".skynet");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.SKYNET_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$SKYNET_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via SKYNET_STATE_DIR.
