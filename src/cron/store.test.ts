...
async function makeStorePath() {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "skynet-cron-store-"));
  return {
    dir,
...
describe("resolveCronStorePath", () => {
  ...
  it("uses SKYNET_HOME for tilde expansion", () => {
    vi.stubEnv("SKYNET_HOME", "/srv/skynet-home");
    vi.stubEnv("HOME", "/home/other");

    const result = resolveCronStorePath("~/cron/jobs.json");
    expect(result).toBe(path.resolve("/srv/skynet-home", "cron", "jobs.json"));
  });
});
