...
describe("resolveGatewayConnection", () => {
  ...
  it("uses SKYNET_GATEWAY_TOKEN for local mode", () => {
    loadConfig.mockReturnValue({ gateway: { mode: "local" } });

    withEnv({ SKYNET_GATEWAY_TOKEN: "env-token" }, () => {
      const result = resolveGatewayConnection({});
  ...
  it("prefers SKYNET_GATEWAY_PASSWORD over remote password fallback", () => {
    ...
    withEnv({ SKYNET_GATEWAY_PASSWORD: "env-pass" }, () => {
      const result = resolveGatewayConnection({});
