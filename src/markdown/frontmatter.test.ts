...
`;
    ...
    expect(parsed.skynet?.emoji).toBe("disk");
  });

  it("preserves inline JSON values", () => {
    const content = `---
name: inline-json
metadata: {"skynet": {"events": ["test"]}}
---
`;
    const result = parseFrontmatterBlock(content);
    expect(result.metadata).toBe('{"skynet": {"events": ["test"]}}');
  });
...
`;
    ...
    const parsed = JSON5.parse(result.metadata ?? "");
    expect(parsed.skynet?.events).toEqual(["command:new"]);
  });
