...
describe("canvas host state dir defaults", () => {
  it("uses SKYNET_STATE_DIR for the default canvas root", async () => {
    await withStateDirEnv("skynet-canvas-state-", async ({ stateDir }) => {
      ...
      try {
        ...
        expect(indexContents).toContain("Skynet Canvas");
      } finally {
        await handler.close();
      }
    });
