...
async function createDockerSetupSandbox(): Promise<DockerSetupSandbox> {
  const rootDir = await mkdtemp(join(tmpdir(), "skynet-docker-setup-"));
...
function createEnv(
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    SKYNET_GATEWAY_TOKEN: "test-token",
    SKYNET_CONFIG_DIR: join(sandbox.rootDir, "config"),
    SKYNET_WORKSPACE_DIR: join(sandbox.rootDir, "skynet"),
  };
...
describe("docker-setup.sh", () => {
  ...
  it("handles env defaults, home-volume mounts, and apt build args", async () => {
    ...
    const result = runDockerSetup(activeSandbox, {
      SKYNET_DOCKER_APT_PACKAGES: "ffmpeg build-essential",
      SKYNET_EXTRA_MOUNTS: undefined,
      SKYNET_HOME_VOLUME: "skynet-home",
    });
    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("SKYNET_DOCKER_APT_PACKAGES=ffmpeg build-essential");
    expect(envFile).toContain("SKYNET_EXTRA_MOUNTS=");
    expect(envFile).toContain("SKYNET_HOME_VOLUME=skynet-home");
    const extraCompose = await readFile(
      join(activeSandbox.rootDir, "docker-compose.extra.yml"),
      "utf8",
    );
    expect(extraCompose).toContain("skynet-home:/home/node");
    expect(extraCompose).toContain("volumes:");
    expect(extraCompose).toContain("skynet-home:");
    const log = await readFile(activeSandbox.logPath, "utf8");
    expect(log).toContain("--build-arg SKYNET_DOCKER_APT_PACKAGES=ffmpeg build-essential");
  });
  ...
  it("precreates config identity dir for CLI device auth writes", async () => {
    ...
    const result = runDockerSetup(activeSandbox, {
      SKYNET_CONFIG_DIR: configDir,
      SKYNET_WORKSPACE_DIR: workspaceDir,
    });
  ...
  it("rejects injected multiline SKYNET_EXTRA_MOUNTS values", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      SKYNET_EXTRA_MOUNTS: "/tmp:/tmp\n  evil-service:\n    image: alpine",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("SKYNET_EXTRA_MOUNTS cannot contain control characters");
  });

  it("rejects invalid SKYNET_EXTRA_MOUNTS mount format", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      SKYNET_EXTRA_MOUNTS: "bad mount spec",
    });
  ...
  it("rejects invalid SKYNET_HOME_VOLUME names", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      SKYNET_HOME_VOLUME: "bad name",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("SKYNET_HOME_VOLUME must match");
  });
