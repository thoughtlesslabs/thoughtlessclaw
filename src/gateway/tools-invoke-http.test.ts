...
let lastCreateSkynetToolsContext: Record<string, unknown> | undefined;
...
vi.mock("../agents/skynet-tools.js", () => {
  ...
  const tools = [
    ...
    },
    {
      ...
      execute: async () => ({
        ok: true,
        route: {
          agentTo: lastCreateSkynetToolsContext?.agentTo,
          agentThreadId: lastCreateSkynetToolsContext?.agentThreadId,
        },
      }),
    },
    {
  ...
  return {
    createSkynetTools: (ctx: Record<string, unknown>) => {
      lastCreateSkynetToolsContext = ctx;
      return tools;
    },
  };
});
...
beforeEach(() => {
  ...
  pluginHttpHandlers = [];
  cfg = {};
  lastCreateSkynetToolsContext = undefined;
});
...
describe("POST /tools/invoke", () => {
  ...
  it("propagates message target/thread headers into tools context for sessions_spawn", async () => {
    ...
    const res = await invokeTool({
      ...
      headers: {
        ...gatewayAuthHeaders(),
        "x-skynet-message-to": "channel:24514",
        "x-skynet-thread-id": "thread-24514",
      },
      tool: "sessions_spawn",
      sessionKey: "main",
    });
