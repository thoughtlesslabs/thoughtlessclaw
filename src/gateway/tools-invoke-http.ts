...
export async function handleToolsInvokeHttpRequest(
  ...
  const messageChannel = normalizeMessageChannel(
    getHeader(req, "x-skynet-message-channel") ?? "",
  );
  const accountId = getHeader(req, "x-skynet-account-id")?.trim() || undefined;
  const agentTo = getHeader(req, "x-skynet-message-to")?.trim() || undefined;
  const agentThreadId = getHeader(req, "x-skynet-thread-id")?.trim() || undefined;
  ...
  const allTools = createSkynetTools({
