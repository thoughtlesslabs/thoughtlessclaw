...
const { createSkynetTools } = await import("../agents/skynet-tools.js");
...
type SessionSendTool = ReturnType<typeof createSkynetTools>[number];
...
function getSessionsSendTool(): SessionSendTool {
  ...
  const tool = createSkynetTools().find((candidate) => candidate.name === "sessions_send");
  if (!tool) {
...
beforeAll(async () => {
  envSnapshot = captureEnv(["SKYNET_GATEWAY_PORT", "SKYNET_GATEWAY_TOKEN"]);
  gatewayPort = await getFreePort();
  testState.gatewayAuth = { mode: "token", token: gatewayToken };
  process.env.SKYNET_GATEWAY_PORT = String(gatewayPort);
  process.env.SKYNET_GATEWAY_TOKEN = gatewayToken;
...
describe("sessions_send label lookup", () => {
  it(
    ...
    async () => {
      ...
      const configPath = process.env.SKYNET_CONFIG_PATH;
      if (!configPath) {
        throw new Error("SKYNET_CONFIG_PATH missing in gateway test environment");
      }
