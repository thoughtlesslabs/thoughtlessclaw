...
const GATEWAY_TEST_ENV_KEYS = [
  ...
  "SKYNET_STATE_DIR",
  "SKYNET_CONFIG_PATH",
  "SKYNET_SKIP_BROWSER_CONTROL_SERVER",
  "SKYNET_SKIP_GMAIL_WATCHER",
  "SKYNET_SKIP_CANVAS_HOST",
  "SKYNET_BUNDLED_PLUGINS_DIR",
  "SKYNET_SKIP_CHANNELS",
  "SKYNET_SKIP_PROVIDERS",
  "SKYNET_SKIP_CRON",
  "SKYNET_TEST_MINIMAL_GATEWAY",
] as const;
...
async function setupGatewayTestHome() {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "skynet-gateway-home-"));
...
function applyGatewaySkipEnv() {
  process.env.SKYNET_SKIP_BROWSER_CONTROL_SERVER = "1";
  process.env.SKYNET_SKIP_GMAIL_WATCHER = "1";
  process.env.SKYNET_SKIP_CANVAS_HOST = "1";
  process.env.SKYNET_SKIP_CHANNELS = "1";
  process.env.SKYNET_SKIP_PROVIDERS = "1";
  process.env.SKYNET_SKIP_CRON = "1";
  process.env.SKYNET_TEST_MINIMAL_GATEWAY = "1";
  process.env.SKYNET_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "skynet-test-no-bundled-extensions")
    : "skynet-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  if (options.uniqueConfigRoot) {
    tempConfigRoot = await fs.mkdtemp(path.join(tempHome, "skynet-test-"));
  } else {
    tempConfigRoot = path.join(tempHome, ".skynet-test");
...
const CONNECT_CHALLENGE_NONCE_KEY = "__skynetTestConnectChallengeNonce";
const CONNECT_CHALLENGE_TRACKED_KEY = "__skynetTestConnectChallengeTracked";
...
export async function startServerWithClient(
  ...
  const envSnapshot = captureEnv(["SKYNET_GATEWAY_TOKEN"]);
  const prev = process.env.SKYNET_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.SKYNET_GATEWAY_TOKEN = fallbackToken;
  }
