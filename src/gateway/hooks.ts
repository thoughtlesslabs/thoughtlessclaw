...
export function resolveHooksConfig(cfg: SkynetConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: SkynetConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-skynet-token"] === "string"
      ? req.headers["x-skynet-token"].trim()
      : "";
  if (headerToken) {
    return headerToken;
  }
