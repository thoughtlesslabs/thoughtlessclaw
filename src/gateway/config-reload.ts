...
import type { SkynetConfig, ConfigFileSnapshot, GatewayReloadMode } from "../config/config.js";
...
export function resolveGatewayReloadSettings(cfg: SkynetConfig): GatewayReloadSettings {
...
export function startGatewayConfigReloader(opts: {
  initialConfig: SkynetConfig;
  readSnapshot: () => Promise<ConfigFileSnapshot>;
  onHotReload: (plan: GatewayReloadPlan, nextConfig: SkynetConfig) => Promise<void>;
  onRestart: (plan: GatewayReloadPlan, nextConfig: SkynetConfig) => void;
  log: {
  ...
  const queueRestart = (plan: GatewayReloadPlan, nextConfig: SkynetConfig) => {
    if (restartQueued) {
      return;
    }
    restartQueued = true;
  ...
  const applySnapshot = async (nextConfig: SkynetConfig) => {
