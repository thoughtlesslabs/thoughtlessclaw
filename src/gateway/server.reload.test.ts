...
describe("gateway hot reload", () => {
  ...
  beforeEach(() => {
    prevSkipChannels = process.env.SKYNET_SKIP_CHANNELS;
    prevSkipGmail = process.env.SKYNET_SKIP_GMAIL_WATCHER;
    prevSkipProviders = process.env.SKYNET_SKIP_PROVIDERS;
    process.env.SKYNET_SKIP_CHANNELS = "0";
    delete process.env.SKYNET_SKIP_GMAIL_WATCHER;
    delete process.env.SKYNET_SKIP_PROVIDERS;
  });
  ...
  afterEach(() => {
    if (prevSkipChannels === undefined) {
      ...
      process.env.SKYNET_SKIP_CHANNELS = prevSkipChannels;
    }
    if (prevSkipGmail === undefined) {
      delete process.env.SKYNET_SKIP_GMAIL_WATCHER;
    } else {
      process.env.SKYNET_SKIP_GMAIL_WATCHER = prevSkipGmail;
    }
    if (prevSkipProviders === undefined) {
      delete process.env.SKYNET_SKIP_PROVIDERS;
    } else {
      process.env.SKYNET_SKIP_PROVIDERS = prevSkipProviders;
    }
  });
