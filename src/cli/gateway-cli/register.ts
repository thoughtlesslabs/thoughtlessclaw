      ...
      .addHelpText(
        ...
        () =>
          `\n${theme.heading("Examples:")}\n${formatHelpExamples([
            ["skynet gateway run", "Run the gateway in the foreground."],
            ["skynet gateway status", "Show service status and probe reachability."],
            ["skynet gateway discover", "Find local and wide-area gateway beacons."],
            ["skynet gateway call health", "Call a gateway RPC method directly."],
          ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/gateway", "docs.skynet.ai/cli/gateway")}\n`,
      ),
  );
