...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update Skynet and inspect update channel status")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["skynet update", "Update a source checkout (git)"],
        ["skynet update --channel beta", "Switch to beta channel (git + npm)"],
        ["skynet update --channel dev", "Switch to dev channel (git + npm)"],
        ["skynet update --tag beta", "One-off update to a dist-tag or version"],
        ["skynet update --dry-run", "Preview actions without changing anything"],
        ["skynet update --no-restart", "Update without restarting the service"],
        ["skynet update --json", "Output result as JSON"],
        ["skynet update --yes", "Non-interactive (accept downgrade prompts)"],
        ["skynet update wizard", "Interactive update wizard"],
        ["skynet --update", "Shorthand for skynet update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run skynet update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.skynet.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["skynet update status", "Show channel + version status."],
          ["skynet update status --json", "JSON output."],
          ["skynet update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.skynet.ai/cli/update")}`,
    )
    .action(async (opts, command) => {
      try {
