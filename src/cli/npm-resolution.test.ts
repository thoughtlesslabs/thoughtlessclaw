...
describe("npm-resolution helpers", () => {
  it("keeps original spec when pin is disabled", () => {
    const result = resolvePinnedNpmSpec({
      rawSpec: "@skynet/plugin-alpha@latest",
      pin: false,
      resolvedSpec: "@skynet/plugin-alpha@1.2.3",
    });
    expect(result).toEqual({
      recordSpec: "@skynet/plugin-alpha@latest",
    });
  });
  ...
  it("returns pinned spec notice when resolved spec is available", () => {
    ...
    expect(result).toEqual({
      recordSpec: "@skynet/plugin-alpha@1.2.3",
      pinNotice: "Pinned npm install record to @skynet/plugin-alpha@1.2.3.",
    });
  });
  ...
  it("maps npm resolution metadata to install fields", () => {
    expect(
      ...
      resolvedName: "@skynet/plugin-alpha",
  ...
  it("builds common npm install record fields", () => {
    expect(
      buildNpmInstallRecordFields({
        spec: "@skynet/plugin-alpha@1.2.3",
        installPath: "/tmp/skynet/extensions/alpha",
        version: "1.2.3",
        resolution: {
          name: "@skynet/plugin-alpha",
          version: "1.2.3",
  ...
  it("resolves pinned install record and emits pin notice", () => {
    ...
    expect(logs).toEqual(["Pinned npm install record to @skynet/plugin-alpha@1.2.3."]);
  ...
  it("resolves pinned install record for CLI and formats warning output", () => {
    ...
    const record = resolvePinnedNpmInstallRecordForCli(
      "@skynet/plugin-alpha@latest",
      true,
      "/tmp/skynet/extensions/alpha",
      "1.2.3",
      undefined,
      (message) => logs.push(message),
    ...
    expect(record).toEqual({
      source: "npm",
      spec: "@skynet/plugin-alpha@latest",
      installPath: "/tmp/skynet/extensions/alpha",
      version: "1.2.3",
