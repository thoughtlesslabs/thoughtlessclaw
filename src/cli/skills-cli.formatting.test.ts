...
describe("skills-cli (e2e)", () => {
  ...
  beforeAll(() => {
    ...
    tempWorkspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "skynet-skills-test-"));
    tempBundledDir = fs.mkdtempSync(path.join(os.tmpdir(), "skynet-bundled-skills-test-"));
    process.env.SKYNET_BUNDLED_SKILLS_DIR = tempBundledDir;
  });
