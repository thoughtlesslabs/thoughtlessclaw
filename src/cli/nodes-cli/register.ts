    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["skynet nodes status", "List known nodes with live status."],
          ["skynet nodes pairing pending", "Show pending node pairing requests."],
          ['skynet nodes run --node <id> --raw "uname -a"', "Run a shell command on a node."],
          ["skynet nodes camera snap --node <id>", "Capture a photo from a node camera."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/nodes", "docs.skynet.ai/cli/nodes")}\n`,
    );
