    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["skynet memory status", "Show index and provider status."],
          ["skynet memory index --force", "Force a full reindex."],
          ['skynet memory search --query "deployment notes"', "Search indexed memory entries."],
          ["skynet memory status --json", "Output machine-readable JSON."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/memory", "docs.skynet.ai/cli/memory")}\n`,
    );
