...
import type { ConfigFileSnapshot, SkynetConfig } from "../config/types.js";

/**
 * Test for issue #6070:
 * `skynet config set/unset` must update snapshot.resolved (user config after $include/${ENV},
 * but before runtime defaults), so runtime defaults don't leak into the written config.
 */

const mockReadConfigFileSnapshot = vi.fn<() => Promise<ConfigFileSnapshot>>();
const mockWriteConfigFile = vi.fn<
  (cfg: SkynetConfig, options?: { unsetPaths?: string[][] }) => Promise<void>
>(async () => {});

vi.mock("../config/config.js", () => ({
  readConfigFileSnapshot: () => mockReadConfigFileSnapshot(),
  writeConfigFile: (cfg: SkynetConfig, options?: { unsetPaths?: string[][] }) =>
...
function setSnapshot(resolved: SkynetConfig, config: SkynetConfig) {
...
describe("config cli", () => {
  ...
  describe("config set - issue #6070", () => {
    it("preserves existing config keys when setting a new value", async () => {
      ...
      const runtimeMerged: SkynetConfig = {
        ...resolved,
        agents: {
          ...resolved.agents,
          defaults: {
