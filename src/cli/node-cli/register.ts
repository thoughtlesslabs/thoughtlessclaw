    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          [
            "skynet node run --host 127.0.0.1 --port 18789",
            "Run the node host in the foreground.",
          ],
          ["skynet node status", "Check node host service status."],
          ["skynet node install", "Install the node host service."],
          ["skynet node restart", "Restart the installed node host service."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/node", "docs.skynet.ai/cli/node")}\n`,
    );
