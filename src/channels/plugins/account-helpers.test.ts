...
function cfg(accounts?: Record<string, unknown> | null): SkynetConfig {
  if (accounts === null) {
    return { channels: { testchannel: {} } } as unknown as SkynetConfig;
  }
  if (accounts === undefined) {
    return {} as unknown as SkynetConfig;
  }
  return { channels: { testchannel: { accounts } } } as unknown as SkynetConfig;
}

describe("createAccountListHelpers", () => {
  describe("listConfiguredAccountIds", () => {
    it("returns empty for missing config", () => {
      expect(listConfiguredAccountIds({} as SkynetConfig)).toEqual([]);
    });
  ...
  describe("listAccountIds", () => {
    it('returns ["default"] for empty config', () => {
      expect(listAccountIds({} as SkynetConfig)).toEqual(["default"]);
    });
  ...
  describe("resolveDefaultAccountId", () => {
    ...
    it('returns "default" for empty config', () => {
      expect(resolveDefaultAccountId({} as SkynetConfig)).toBe("default");
    });
  });
});
