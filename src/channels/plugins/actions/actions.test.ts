...
function telegramCfg(): SkynetConfig {
  return { channels: { telegram: { botToken: "tok" } } } as SkynetConfig;
}
...
async function runTelegramAction(
  ...
  options?: { cfg?: SkynetConfig; accountId?: string },
) {
...
async function runSignalAction(
  ...
  const cfg =
    options?.cfg ?? ({ channels: { signal: { account: "+15550001111" } } } as SkynetConfig);
...
function slackHarness() {
  const cfg = { channels: { slack: { botToken: "tok" } } } as SkynetConfig;
...
function createSignalAccountOverrideCfg(): SkynetConfig {
  return {
    channels: {
      signal: {
...
describe("discord message actions", () => {
  it("lists channel and upload actions by default", async () => {
    const cfg = { channels: { discord: { token: "d0" } } } as SkynetConfig;
  ...
  it("lists moderation when at least one account enables it", () => {
    ...
    for (const channelConfig of cases) {
      const cfg = channelConfig as unknown as SkynetConfig;
      const actions = discordMessageActions.listActions?.({ cfg }) ?? [];
  ...
  it("maps reaction targets into signal sendReaction calls", async () => {
    const cases = [
      ...
      },
      {
        ...
        cfg: { channels: { signal: { account: "+15550001111" } } } as SkynetConfig,
        accountId: undefined,
        params: {
