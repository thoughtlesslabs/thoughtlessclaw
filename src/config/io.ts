...
import type { SkynetConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
  validateConfigObjectRawWithPlugins,
  validateConfigObjectWithPlugins,
} from "./validation.js";
import { compareSkynetVersions } from "./version.js";
...
function formatConfigValidationFailure(pathLabel: string, issueMessage: string): string {
  ...
  return [
    ...
    "",
    "Fix with:",
    `  skynet config set ${allowPath} '["*"]'`,
    "",
    "Or switch policy:",
    `  skynet config set ${policyPath} "pairing"`,
  ].join("\n");
}
...
function unsetPathForWrite(
  root: SkynetConfig,
  pathSegments: string[],
): { changed: boolean; next: SkynetConfig } {
...
function coerceConfig(value: unknown): SkynetConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as SkynetConfig;
}
...
function stampConfigVersion(cfg: SkynetConfig): SkynetConfig {
...
function warnIfConfigFromFuture(cfg: SkynetConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) {
    return;
  }
  const cmp = compareSkynetVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer Skynet (${touched}); current version is ${VERSION}.`,
    );
  }
}
...
function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as SkynetConfig, env);
  }
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): SkynetConfig {
    try {
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as SkynetConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: SkynetConfig, options: ConfigWriteOptions = {}) {
    clearConfigCache();
    ...
    const outputConfigBase =
      envRefMap && changedPaths
        ? (restoreEnvRefsFromMap(cfgToWrite, "", envRefMap, changedPaths) as SkynetConfig)
        : cfgToWrite;
    ...
    const logConfigOverwrite = () => {
      ...
      const shouldLogInVitest = deps.env.SKYNET_TEST_CONFIG_OVERWRITE_LOG === "1";
    ...
    const logConfigWriteAnomalies = () => {
      ...
      const isVitest = deps.env.VITEST === "true";
      const shouldLogInVitest = deps.env.SKYNET_TEST_CONFIG_WRITE_ANOMALY_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
    ...
    const auditRecordBase = {
      ...
      watchMode: deps.env.SKYNET_WATCH_MODE === "1",
      watchSession:
        typeof deps.env.SKYNET_WATCH_SESSION === "string" &&
        deps.env.SKYNET_WATCH_SESSION.trim().length > 0
          ? deps.env.SKYNET_WATCH_SESSION.trim()
          : null,
      watchCommand:
        typeof deps.env.SKYNET_WATCH_COMMAND === "string" &&
        deps.env.SKYNET_WATCH_COMMAND.trim().length > 0
          ? deps.env.SKYNET_WATCH_COMMAND.trim()
          : null,
      existsBefore: snapshot.exists,
...
// module scope. `SKYNET_CONFIG_PATH` (and friends) are expected to work even
...
function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.SKYNET_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.SKYNET_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
