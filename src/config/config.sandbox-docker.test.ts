...
describe("sandbox browser binds config", () => {
  ...
  it("defaults browser network to dedicated sandbox network", () => {
    ...
    expect(resolved.network).toBe("skynet-sandbox-browser");
  });

  it("prefers agent browser network over global browser network", () => {
    const resolved = resolveSandboxBrowserConfig({
      scope: "agent",
      globalBrowser: { network: "skynet-sandbox-browser-global" },
      agentBrowser: { network: "skynet-sandbox-browser-agent" },
    });
    expect(resolved.network).toBe("skynet-sandbox-browser-agent");
  });
