...
import type { ConfigFileSnapshot } from "./types.skynet.js";
...
function makeSnapshot<TConfig extends Record<string, unknown>>(
  ...
  return {
    path: "/home/user/.skynet/config.json5",
    exists: true,
...
describe("redactConfigSnapshot", () => {
  ...
  it("does not redact passwordFile path fields", () => {
    const snapshot = makeSnapshot({
      channels: {
        irc: {
          passwordFile: "/etc/skynet/irc-password.txt",
          nickserv: {
            passwordFile: "/etc/skynet/nickserv-password.txt",
    ...
    expect(irc.passwordFile).toBe("/etc/skynet/irc-password.txt");
    expect(nickserv.passwordFile).toBe("/etc/skynet/nickserv-password.txt");
  ...
  it("contract-covers dynamic catchall/record paths for redact+restore", () => {
    const hints = mapSensitivePaths(SkynetSchema, "", {});
    const snapshot = makeSnapshot({
      env: {
