...
import { scheduleGatewaySigusr1Restart, triggerSkynetRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting Skynet in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerSkynetRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting Skynet via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
