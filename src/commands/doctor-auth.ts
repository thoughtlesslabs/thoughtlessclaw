...
function pruneAuthProfiles(
  cfg: SkynetConfig,
  profileIds: Set<string>,
): { next: SkynetConfig; changed: boolean } {
...
export async function maybeRemoveDeprecatedCliAuthProfiles(
  ...
  if (deprecated.has(CLAUDE_CLI_PROFILE_ID)) {
    lines.push(
      `- ${CLAUDE_CLI_PROFILE_ID} (Anthropic): use setup-token → ${formatCliCommand("skynet models auth setup-token")}`,
    );
  }
...
function formatAuthIssueHint(issue: AuthIssue): string | null {
  if (issue.provider === "anthropic" && issue.profileId === CLAUDE_CLI_PROFILE_ID) {
    return `Deprecated profile. Use ${formatCliCommand("skynet models auth setup-token")} or ${formatCliCommand(
      "skynet configure",
    )}.`;
  }
  if (issue.provider === "openai-codex" && issue.profileId === CODEX_CLI_PROFILE_ID) {
    return `Deprecated profile. Use ${formatCliCommand(
      "skynet models auth login --provider openai-codex",
    )} or ${formatCliCommand("skynet configure")}.`;
  }
  return `Re-auth via \`${formatCliCommand("skynet configure")}\` or \`${formatCliCommand("skynet onboard")}\`.`;
}
