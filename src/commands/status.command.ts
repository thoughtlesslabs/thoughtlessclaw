...
export async function statusCommand(
  ...
  runtime.log(theme.heading("Skynet status"));
  runtime.log("");
  ...
  if (pairingRecovery) {
    ...
    if (pairingRecovery.requestId) {
      runtime.log(
        theme.muted(
          `Recovery: ${formatCliCommand(`skynet devices approve ${pairingRecovery.requestId}`)}`,
        ),
      );
    }
    runtime.log(theme.muted(`Fallback: ${formatCliCommand("skynet devices approve --latest")}`));
    runtime.log(theme.muted(`Inspect: ${formatCliCommand("skynet devices list")}`));
  }
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("skynet security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("skynet security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.skynet.ai/faq");
  runtime.log("Troubleshooting: https://docs.skynet.ai/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("skynet status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("skynet logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("skynet status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("skynet gateway probe")}`);
  }
}
