...
const ensureSkynetModelsJson = vi.fn().mockResolvedValue(undefined);
const ensurePiAuthJsonFromAuthProfiles = vi
  .fn()
  .mockResolvedValue({ wrote: false, authPath: "/tmp/skynet-agent/auth.json" });
const resolveSkynetAgentDir = vi.fn().mockReturnValue("/tmp/skynet-agent");
...
vi.mock("../config/config.js", () => ({
  ...
  STATE_DIR: "/tmp/skynet-state",
  loadConfig,
}));

vi.mock("../agents/models-config.js", () => ({
  ensureSkynetModelsJson,
}));
...
describe("models list/status", () => {
  ...
  it("models list syncs auth-profiles into auth.json before availability checks", async () => {
    ...
    expect(ensurePiAuthJsonFromAuthProfiles).toHaveBeenCalledWith("/tmp/skynet-agent");
  });
