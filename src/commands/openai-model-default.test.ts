...
function expectPrimaryModelChanged(
  applied: { changed: boolean; next: SkynetConfig },
  primary: string,
) {
...
type SharedDefaultModelCase = {
  apply: (cfg: SkynetConfig) => { changed: boolean; next: SkynetConfig };
  defaultModel: string;
  overrideConfig: SkynetConfig;
  alreadyDefaultConfig: SkynetConfig;
};
...
describe("applyDefaultModelChoice", () => {
  it("ensures allowlist entry exists when returning an agent override", async () => {
    ...
    const applied = await applyDefaultModelChoice({
      ...
      applyProviderConfig: (config: SkynetConfig) => config,
      applyDefaultConfig: (config: SkynetConfig) => config,
      noteAgentModel,
...
describe("shared default model behavior", () => {
  it("sets defaults when model is unset", () => {
    for (const testCase of SHARED_DEFAULT_MODEL_CASES) {
      const cfg: SkynetConfig = { agents: { defaults: {} } };
