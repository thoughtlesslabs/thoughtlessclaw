...
export async function setupAuthTestEnv(
  ...
  process.env.SKYNET_AGENT_DIR = agentDir;
...
export function requireSkynetAgentDir(): string {
  const agentDir = process.env.SKYNET_AGENT_DIR;
  if (!agentDir) {
    throw new Error("SKYNET_AGENT_DIR not set");
  }
  return agentDir;
}
