...
const SEARCH_TOOL = "https://docs.skynet.ai/mcp.SearchSkynet";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.skynet.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('skynet docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.skynet.ai/");
      runtime.log(`Search: ${formatCliCommand('skynet docs "your query"')}`);
    }
    return;
  }
