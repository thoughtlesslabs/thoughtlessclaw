...
describe("doctor command", () => {
  ...
  it("warns when the state directory is missing", async () => {
    ...
    const missingDir = fs.mkdtempSync(path.join(os.tmpdir(), "skynet-missing-state-"));
    fs.rmSync(missingDir, { recursive: true, force: true });
    process.env.SKYNET_STATE_DIR = missingDir;
    note.mockClear();
  ...
  it("skips gateway auth warning when SKYNET_GATEWAY_TOKEN is set", async () => {
    mockDoctorConfigSnapshot({
      config: {
        gateway: { mode: "local" },
      },
    ...
    process.env.SKYNET_GATEWAY_TOKEN = "env-token-1234567890";
    note.mockClear();
