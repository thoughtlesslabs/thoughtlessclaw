...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveSkynetAgentDir: vi.fn().mockReturnValue("/tmp/skynet-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/skynet-agent"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/skynet-agent/auth-profiles.json"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveSkynetAgentDir: mocks.resolveSkynetAgentDir,
}));
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/skynet-agent");
    }
  }
}
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveSkynetAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.auth.storePath).toBe("/tmp/skynet-agent/auth-profiles.json");
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/skynet-agent-custom",
      },
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/skynet-agent-custom");
