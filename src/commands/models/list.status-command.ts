...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveSkynetAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("skynet models auth setup-token")}\` or \`${formatCliCommand("skynet configure")}\`.`
          : `Run \`${formatCliCommand("skynet configure")}\` or set an API key env var.`;
