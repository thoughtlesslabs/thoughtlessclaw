...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "SKYNET_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("SKYNET_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv SKYNET_GATEWAY_TOKEN");
    expect(message).not.toContain("SKYNET_GATEWAY_PASSWORD");
  });
