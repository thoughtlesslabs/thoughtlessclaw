...
function resolveAgentName(cfg: SkynetConfig, agentId: string) {
...
function resolveAgentModel(cfg: SkynetConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: SkynetConfig): AgentSummary[] {
