...
type PromptDefaultModelResult = { model?: string; config?: SkynetConfig };
...
function resolveConfiguredModelRaw(cfg: SkynetConfig): string {
  return resolveAgentModelPrimaryValue(cfg.agents?.defaults?.model) ?? "";
}

function resolveConfiguredModelKeys(cfg: SkynetConfig): string[] {
...
export function applyPrimaryModel(cfg: SkynetConfig, model: string): SkynetConfig {
...
export function applyModelAllowlist(cfg: SkynetConfig, models: string[]): SkynetConfig {
  const defaults = cfg.agents?.defaults;
