...
function resolveMode(cfg: SkynetConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  const root = await resolveSkynetPackageRoot({
  ...
  let cfg: SkynetConfig = configResult.cfg;
  ...
  if (!cfg.gateway?.mode) {
    const lines = [
      ...
      `Fix: run ${formatCliCommand("skynet configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("skynet config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("skynet setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("skynet doctor --fix")}" to apply changes.`);
  }
