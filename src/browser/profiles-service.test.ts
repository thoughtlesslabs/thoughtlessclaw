...
vi.mock("./chrome.js", () => ({
  resolveSkynetUserDataDir: vi.fn(() => "/tmp/skynet-test/skynet/user-data"),
}));

import { loadConfig, writeConfigFile } from "../config/config.js";
import { resolveSkynetUserDataDir } from "./chrome.js";
...
describe("BrowserProfilesService", () => {
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "skynet-profile-"));
    const userDataDir = path.join(tempDir, "work", "user-data");
    fs.mkdirSync(path.dirname(userDataDir), { recursive: true });
    vi.mocked(resolveSkynetUserDataDir).mockReturnValue(userDataDir);
