...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredSkynetTmpDir: vi.fn(() => "/tmp/skynet"),
}));
vi.mock("../infra/tmp-skynet-dir.js", () => tmpDirMocks);
...
describe("pw-tools-core", () => {
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredSkynetTmpDir.mockReturnValue("/tmp/skynet");
  });
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredSkynetTmpDir.mockReturnValue("/tmp/skynet-preferred");
    ...
    const expectedDownloadsTail = `${path.join("tmp", "skynet-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredSkynetTmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(String(outPath))).toBe(
      path.join(path.sep, "tmp", "skynet-preferred", "downloads"),
    );
    expect(path.basename(String(outPath))).toMatch(/-passwd$/);
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "skynet-preferred", "downloads")}${path.sep}`),
    );
  });
