  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        profiles: {
          skynet: { cdpPort: testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
describe("browser control evaluate gating", () => {
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.SKYNET_GATEWAY_PORT;
    process.env.SKYNET_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.SKYNET_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.SKYNET_GATEWAY_PASSWORD;
