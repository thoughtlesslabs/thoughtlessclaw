...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".skynet-profile-decorated");
}
 ...
 */
export function decorateSkynetProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_SKYNET_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_SKYNET_BROWSER_COLOR).toUpperCase();
