import type { BrowserConfig, BrowserProfileConfig, SkynetConfig } from "../config/config.js";
...
import {
  ...
  DEFAULT_SKYNET_BROWSER_ENABLED,
...
export type ResolvedBrowserProfile = {
  ...
  cdpIsLoopback: boolean;
  color: string;
  driver: "skynet" | "extension";
};

function normalizeHexColor(raw: string | undefined) {
  const value = (raw ?? "").trim();
  if (!value) {
    return DEFAULT_SKYNET_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "skynet" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_SKYNET_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_SKYNET_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an Skynet browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for skynet/skynet CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: SkynetConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_SKYNET_BROWSER_ENABLED;
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "skynet";
