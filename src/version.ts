...
declare const __SKYNET_VERSION__: string | undefined;
const CORE_PACKAGE_NAME = "skynet";
...
export function resolveRuntimeServiceVersion(
  ...
  return (
    firstNonEmpty(
      env["SKYNET_VERSION"],
      env["SKYNET_SERVICE_VERSION"],
      env["npm_package_version"],
    ) ?? fallback
  );
}

// Single source of truth for the current Skynet version.
// - Embedded/bundled builds: injected define or env var.
// - Dev/npm builds: package.json.
export const VERSION =
  (typeof __SKYNET_VERSION__ === "string" && __SKYNET_VERSION__) ||
  process.env.SKYNET_BUNDLED_VERSION ||
