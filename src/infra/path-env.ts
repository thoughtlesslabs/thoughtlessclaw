...
function candidateBinDirs(opts: EnsureSkynetPathOpts): { prepend: string[]; append: string[] } {
  ...
  // Bundled macOS app: `skynet` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "skynet");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.SKYNET_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "skynet"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `skynet` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureSkynetCliOnPath(opts: EnsureSkynetPathOpts = {}) {
  if (isTruthyEnvValue(process.env.SKYNET_PATH_BOOTSTRAPPED)) {
    return;
  }
