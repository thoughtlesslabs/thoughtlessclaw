...
let ensureSkynetCliOnPath: typeof import("./path-env.js").ensureSkynetCliOnPath;

describe("ensureSkynetCliOnPath", () => {
  const envKeys = [
    "PATH",
    "SKYNET_PATH_BOOTSTRAPPED",
    "SKYNET_ALLOW_PROJECT_LOCAL_BIN",
    "MISE_DATA_DIR",
    "HOMEBREW_PREFIX",
    "HOMEBREW_BREW_FILE",
    "XDG_BIN_HOME",
  ...
  beforeAll(async () => {
    ({ ensureSkynetCliOnPath } = await import("./path-env.js"));
  });
  ...
  it("prepends the bundled app bin dir when a sibling skynet exists", () => {
    const tmp = abs("/tmp/skynet-path/case-bundled");
    const appBinDir = path.join(tmp, "AppBin");
    const cliPath = path.join(appBinDir, "skynet");
    setDir(tmp);
    setDir(appBinDir);
    setExe(cliPath);

    process.env.PATH = "/usr/bin";
    delete process.env.SKYNET_PATH_BOOTSTRAPPED;
  ...
  it("is idempotent", () => {
    ...
    process.env.SKYNET_PATH_BOOTSTRAPPED = "1";
    ensureSkynetCliOnPath({
  ...
  it("prepends mise shims when available", () => {
    const tmp = abs("/tmp/skynet-path/case-mise");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "skynet");
    setDir(tmp);
    setDir(appBinDir);
    setExe(appCli);
  ...
  it("only appends project-local node_modules/.bin when explicitly enabled", () => {
    const tmp = abs("/tmp/skynet-path/case-project-local");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "skynet");
    setDir(tmp);
    setDir(appBinDir);
    ...
    const localCli = path.join(localBinDir, "skynet");
  ...
  it("prepends Linuxbrew dirs when present", () => {
    const tmp = abs("/tmp/skynet-path/case-linuxbrew");
