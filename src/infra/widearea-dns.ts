...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.SKYNET_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*skynet-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "skynet";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "skynet");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "skynet-gw");
  ...
  records.push(`_skynet-gw._tcp IN PTR ${instanceLabel}._skynet-gw._tcp`);
  records.push(`${instanceLabel}._skynet-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._skynet-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; skynet-content-hash: ${contentHash}\n${contentBody}`;
}
