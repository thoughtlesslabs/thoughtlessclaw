...
vi.mock("./skynet-root.js", () => ({
  resolveSkynetPackageRoot: vi.fn(async () => null),
  resolveSkynetPackageRootSync: vi.fn(() => null),
}));
...
let skynetRoot: typeof import("./skynet-root.js");
...
describe("control UI assets helpers (fs-mocked)", () => {
  beforeAll(async () => {
    ...
    skynetRoot = await import("./skynet-root.js");
  });
  ...
  it("uses resolveSkynetPackageRoot when available", async () => {
    const pkgRoot = abs("fixtures/skynet");
    (
      skynetRoot.resolveSkynetPackageRoot as unknown as ReturnType<typeof vi.fn>
    ).mockResolvedValueOnce(pkgRoot);

    await expect(resolveControlUiDistIndexPath(abs("fixtures/bin/skynet"))).resolves.toBe(
  ...
  it("falls back to package.json name matching when root resolution fails", async () => {
    ...
    setFile(path.join(root, "package.json"), JSON.stringify({ name: "skynet" }));
    setFile(path.join(root, "dist", "control-ui", "index.html"), "<html></html>\n");

    await expect(resolveControlUiDistIndexPath(path.join(root, "skynet.mjs"))).resolves.toBe(
      path.join(root, "dist", "control-ui", "index.html"),
    );
  });

  it("returns null when fallback package name does not match", async () => {
    const root = abs("fixtures/not-skynet");
  ...
  it("resolves control-ui root for dist bundle argv1 and moduleUrl candidates", async () => {
    const pkgRoot = abs("fixtures/skynet-bundle");
    (
      skynetRoot.resolveSkynetPackageRootSync as unknown as ReturnType<typeof vi.fn>
