...
const VITEST_FS_BASE = path.join(path.parse(process.cwd()).root, "__skynet_vitest__");
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "skynet-root");
...
describe("resolveSkynetPackageRoot", () => {
  let resolveSkynetPackageRoot: typeof import("./skynet-root.js").resolveSkynetPackageRoot;
  let resolveSkynetPackageRootSync: typeof import("./skynet-root.js").resolveSkynetPackageRootSync;

  beforeAll(async () => {
    ({ resolveSkynetPackageRoot, resolveSkynetPackageRootSync } =
      await import("./skynet-root.js"));
  });
  ...
  it("resolves package root from .bin argv1", async () => {
    ...
    const argv1 = path.join(project, "node_modules", ".bin", "skynet");
    const pkgRoot = path.join(project, "node_modules", "skynet");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "skynet" }));

    expect(resolveSkynetPackageRootSync({ argv1 })).toBe(pkgRoot);
  });

  it("resolves package root via symlinked argv1", async () => {
    const project = fx("symlink-scenario");
    const bin = path.join(project, "bin", "skynet");
    const realPkg = path.join(project, "real-pkg");
    state.realpaths.set(abs(bin), abs(path.join(realPkg, "skynet.mjs")));
    setFile(path.join(realPkg, "package.json"), JSON.stringify({ name: "skynet" }));

    expect(resolveSkynetPackageRootSync({ argv1: bin })).toBe(realPkg);
  });
  ...
  it("prefers moduleUrl candidates", async () => {
    ...
    expect(resolveSkynetPackageRootSync({ moduleUrl })).toBe(pkgRoot);
  });

  it("returns null for non-skynet package roots", async () => {
    const pkgRoot = fx("not-skynet");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "not-skynet" }));

    expect(resolveSkynetPackageRootSync({ cwd: pkgRoot })).toBeNull();
  });
  ...
  it("async resolver matches sync behavior", async () => {
    ...
    await expect(resolveSkynetPackageRoot({ cwd: pkgRoot })).resolves.toBe(pkgRoot);
  });

  it("async resolver returns null when no package roots exist", async () => {
    await expect(resolveSkynetPackageRoot({ cwd: fx("missing") })).resolves.toBeNull();
  });
});
