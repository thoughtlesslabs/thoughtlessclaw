...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: SkynetConfig | null = null;
  ...
  get(key: string, cfg: SkynetConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: SkynetConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  private resetIfConfigChanged(cfg: SkynetConfig): void {
