...
function getDiagnosticEventsState(): DiagnosticEventsGlobalState {
  const globalStore = globalThis as typeof globalThis & {
    __skynetDiagnosticEventsState?: DiagnosticEventsGlobalState;
  };
  if (!globalStore.__skynetDiagnosticEventsState) {
    globalStore.__skynetDiagnosticEventsState = {
      seq: 0,
      listeners: new Set<(evt: DiagnosticEventPayload) => void>(),
      dispatchDepth: 0,
    };
  }
  return globalStore.__skynetDiagnosticEventsState;
}

export function isDiagnosticsEnabled(config?: SkynetConfig): boolean {
