...
export async function resolveApnsAuthConfigFromEnv(
  ...
  const teamId = normalizeNonEmptyString(env.SKYNET_APNS_TEAM_ID);
  const keyId = normalizeNonEmptyString(env.SKYNET_APNS_KEY_ID);
  if (!teamId || !keyId) {
    return {
      ok: false,
      error: "APNs auth missing: set SKYNET_APNS_TEAM_ID and SKYNET_APNS_KEY_ID",
    };
  }

  const inlineKeyRaw =
    normalizeNonEmptyString(env.SKYNET_APNS_PRIVATE_KEY_P8) ??
    normalizeNonEmptyString(env.SKYNET_APNS_PRIVATE_KEY);
  if (inlineKeyRaw) {
    return {
      ok: true,
      value: {
  ...
  const keyPath = normalizeNonEmptyString(env.SKYNET_APNS_PRIVATE_KEY_PATH);
  if (!keyPath) {
    return {
      ok: false,
      error:
        "APNs private key missing: set SKYNET_APNS_PRIVATE_KEY_P8 or SKYNET_APNS_PRIVATE_KEY_PATH",
    };
  }
  try {
    ...
    return {
      ok: false,
      error: `failed reading SKYNET_APNS_PRIVATE_KEY_PATH (${keyPath}): ${message}`,
    };
  }
}
...
function createSkynetPushMetadata(params: {
...
export async function sendApnsAlert(params: {
  ...
  const payload = {
    ...
    skynet: createSkynetPushMetadata({
      kind: "push.test",
      nodeId: params.nodeId,
    }),
  };
