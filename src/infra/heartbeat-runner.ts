...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: SkynetConfig) => void;
};

function hasExplicitHeartbeatAgents(cfg: SkynetConfig) {
...
export function resolveHeartbeatPrompt(cfg: SkynetConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: SkynetConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: SkynetConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
