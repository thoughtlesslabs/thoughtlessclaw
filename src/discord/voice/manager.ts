...
function resolveVoiceTtsConfig(params: { cfg: SkynetConfig; override?: TtsConfig }): {
  cfg: SkynetConfig;
...
async function writeWavFile(pcm: Buffer): Promise<{ path: string; durationSeconds: number }> {
  const tempDir = await fs.mkdtemp(path.join(resolvePreferredSkynetTmpDir(), "discord-voice-"));
