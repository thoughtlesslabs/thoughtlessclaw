...
const TEMP_MEDIA_PREFIX = "skynet-media-";
...
function createGroqAudioConfig(): SkynetConfig {
  return {
    tools: {
      media: {
        audio: {
...
function createMediaDisabledConfig(): SkynetConfig {
  return {
    tools: {
      media: {
        audio: { enabled: false },
...
function createMediaDisabledConfigWithAllowedMimes(allowedMimes: string[]): SkynetConfig {
  return {
...
async function setupAudioAutoDetectCase(stdout: string): Promise<{
  ...
  const cfg: SkynetConfig = { tools: { media: { audio: {} } } };
...
describe("applyMediaUnderstanding", () => {
  ...
  beforeAll(async () => {
    const baseDir = resolvePreferredSkynetTmpDir();
  ...
  it("skips audio auto-detect when no supported binaries or provider keys are available", async () => {
    ...
    await withMediaAutoDetectEnv(
      {
        PATH: emptyBinDir,
        SKYNET_AGENT_DIR: isolatedAgentDir,
