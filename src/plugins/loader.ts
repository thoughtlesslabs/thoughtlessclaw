...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: SkynetPluginDefinition;
  register?: SkynetPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as SkynetPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as SkynetPluginDefinition;
...
export function loadSkynetPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverSkynetPlugins({
    workspaceDir: options.workspaceDir,
  ...
  const getJiti = () => {
    ...
    jitiLoader = createJiti(import.meta.url, {
      ...
      ...(pluginSdkAlias || pluginSdkAccountIdAlias
        ? {
            alias: {
              ...(pluginSdkAlias ? { "skynet/plugin-sdk": pluginSdkAlias } : {}),
              ...(pluginSdkAccountIdAlias
                ? { "skynet/plugin-sdk/account-id": pluginSdkAccountIdAlias }
                : {}),
            },
          }
        : {}),
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: SkynetPluginModule | null = null;
    try {
      mod = getJiti()(candidate.source) as SkynetPluginModule;
    } catch (err) {
      recordPluginError({
        logger,
        registry,
