...
describe("workspace path resolution", () => {
  it("resolves relative read/write/edit paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("skynet-ws-", async (workspaceDir) => {
      await withTempDir("skynet-cwd-", async (otherDir) => {
        const cwdSpy = vi.spyOn(process, "cwd").mockReturnValue(otherDir);
        try {
          const tools = createSkynetCodingTools({ workspaceDir });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("skynet-ws-", async (workspaceDir) => {
      await withTempDir("skynet-override-", async (overrideDir) => {
        const tools = createSkynetCodingTools({
          workspaceDir,
          exec: { host: "gateway", ask: "off", security: "full" },
        });
  ...
  it("rejects @-prefixed absolute paths outside workspace when workspaceOnly is enabled", async () => {
    await withTempDir("skynet-ws-", async (workspaceDir) => {
      const cfg: SkynetConfig = { tools: { fs: { workspaceOnly: true } } };
      const tools = createSkynetCodingTools({ workspaceDir, config: cfg });
      const { readTool } = expectReadWriteEditTools(tools);

      const outsideAbsolute = path.resolve(path.parse(workspaceDir).root, "outside-skynet.txt");
      await expect(
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("skynet-sandbox-", async (sandboxDir) => {
      await withTempDir("skynet-workspace-", async (workspaceDir) => {
        ...
        const tools = createSkynetCodingTools({ workspaceDir, sandbox });
        const { readTool, writeTool, editTool } = expectReadWriteEditTools(tools);
