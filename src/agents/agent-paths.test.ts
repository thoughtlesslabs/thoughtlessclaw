...
describe("resolveSkynetAgentDir", () => {
  const withTempStateDir = async (run: (stateDir: string) => void) => {
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "skynet-agent-"));
    try {
      run(stateDir);
    } finally {
  ...
  it("honors SKYNET_AGENT_DIR overrides", async () => {
    await withTempStateDir((stateDir) => {
      const override = path.join(stateDir, "agent");
      withEnv(
        {
          SKYNET_STATE_DIR: undefined,
          SKYNET_AGENT_DIR: override,
          PI_CODING_AGENT_DIR: undefined,
        },
        () => {
          const resolved = resolveSkynetAgentDir();
  ...
  it("honors PI_CODING_AGENT_DIR when SKYNET_AGENT_DIR is unset", async () => {
    await withTempStateDir((stateDir) => {
  ...
  it("prefers SKYNET_AGENT_DIR over PI_CODING_AGENT_DIR when both are set", async () => {
    await withTempStateDir((stateDir) => {
      ...
      withEnv(
        {
          SKYNET_STATE_DIR: undefined,
          SKYNET_AGENT_DIR: primaryOverride,
