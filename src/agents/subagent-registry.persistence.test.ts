...
describe("subagent registry persistence", () => {
  const envSnapshot = captureEnv(["SKYNET_STATE_DIR"]);
  ...
  const writePersistedRegistry = async (
    ...
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "skynet-subagent-"));
  ...
  it("uses isolated temp state when SKYNET_STATE_DIR is unset in tests", async () => {
    ...
    expect(registryPath).toContain(path.join(os.tmpdir(), "skynet-test-state"));
  });
});
