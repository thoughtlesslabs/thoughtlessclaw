...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses SKYNET_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      SKYNET_HOME: "/srv/skynet-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/skynet-home"), ".skynet", "workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".skynet", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("skynet-workspace-");
