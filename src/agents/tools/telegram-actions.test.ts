...
describe("handleTelegramAction", () => {
  ...
  function reactionConfig(reactionLevel: "minimal" | "extensive" | "off" | "ack"): SkynetConfig {
    return {
      channels: { telegram: { botToken: "tok", reactionLevel } },
    } as SkynetConfig;
  }

  function telegramConfig(overrides?: Record<string, unknown>): SkynetConfig {
    return {
      channels: {
        telegram: {
          botToken: "tok",
  ...
  it("rejects sticker actions when disabled by default", async () => {
    const cfg = { channels: { telegram: { botToken: "tok" } } } as SkynetConfig;
    await expect(
...
describe("handleTelegramAction per-account gating", () => {
  async function expectAccountStickerSend(cfg: SkynetConfig, accountId = "media") {
    await handleTelegramAction(
