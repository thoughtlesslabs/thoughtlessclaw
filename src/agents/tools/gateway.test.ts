...
describe("gateway tool defaults", () => {
  const envSnapshot = {
    skynet: process.env.SKYNET_GATEWAY_TOKEN,
  ...
  afterAll(() => {
    if (envSnapshot.skynet === undefined) {
      delete process.env.SKYNET_GATEWAY_TOKEN;
    } else {
      process.env.SKYNET_GATEWAY_TOKEN = envSnapshot.skynet;
    }
  ...
  it("uses SKYNET_GATEWAY_TOKEN for allowlisted local overrides", () => {
  ...
  it("does not leak local env/config tokens to remote overrides", () => {
    process.env.SKYNET_GATEWAY_TOKEN = "local-env-token";
