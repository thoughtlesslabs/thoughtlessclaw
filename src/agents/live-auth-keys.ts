...
const GOOGLE_LIVE_SINGLE_KEY = "SKYNET_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "SKYNET_LIVE_ANTHROPIC_KEY",
    listVar: "SKYNET_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "SKYNET_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `SKYNET_LIVE_${base}_KEY`;
