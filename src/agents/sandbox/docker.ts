...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "skynet.sandbox=1");
  args.push("--label", `skynet.sessionKey=${params.scopeKey}`);
  args.push("--label", `skynet.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `skynet.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "skynet.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`skynet sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`skynet sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("skynet sandbox recreate --all");
}
