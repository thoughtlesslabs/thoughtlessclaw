...
const defaultTools = createSkynetCodingTools({ senderIsOwner: true });
...
describe("createSkynetCodingTools", () => {
  ...
  it("enforces apply_patch availability and canonical names across model/provider constraints", () => {
    ...
    const enabledConfig: SkynetConfig = {
      tools: {
        exec: {
    ...
    const openAiTools = createSkynetCodingTools({
    ...
    const anthropicTools = createSkynetCodingTools({
      config: enabledConfig,
    ...
    const allowModelsConfig: SkynetConfig = {
      tools: {
        exec: {
    ...
    const allowed = createSkynetCodingTools({
    ...
    const denied = createSkynetCodingTools({
      config: allowModelsConfig,
      modelProvider: "openai",
      modelId: "gpt-5-mini",
    });
    ...
    const oauthTools = createSkynetCodingTools({
      modelProvider: "anthropic",
      modelAuthMode: "oauth",
    });
