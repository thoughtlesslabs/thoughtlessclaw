...
describe("createPiToolsSandboxContext", () => {
  it("provides stable defaults for pi-tools sandbox tests", () => {
    ...
    expect(sandbox.containerName).toBe("skynet-sbx-test");
    expect(sandbox.containerWorkdir).toBe("/workspace");
    expect(sandbox.docker.image).toBe("skynet-sandbox:bookworm-slim");
    expect(sandbox.docker.containerPrefix).toBe("skynet-sbx-");
