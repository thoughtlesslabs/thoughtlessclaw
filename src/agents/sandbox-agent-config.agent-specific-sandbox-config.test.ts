...
async function resolveContext(config: SkynetConfig, sessionKey: string, workspaceDir: string) {
...
function createWorkSetupCommandConfig(scope: "agent" | "shared"): SkynetConfig {
  return {
    agents: {
      defaults: {
        sandbox: {
