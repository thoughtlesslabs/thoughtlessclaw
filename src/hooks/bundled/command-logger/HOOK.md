...
homepage: https://docs.skynet.ai/automation/hooks#command-logger
metadata:
  {
    "skynet":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.skynet/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.skynet/logs/commands.log ~/.skynet/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/skynet`:
   ```
   /home/username/.skynet/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
