...
This directory contains hooks that ship with Skynet. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
Hooks can be configured in `~/.skynet/skynet.json`:
...
2. Restart gateway: `pkill -9 -f 'skynet.*gateway' && pnpm skynet gateway`
3. Enable the hook: `skynet hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.skynet.ai/automation/hooks
