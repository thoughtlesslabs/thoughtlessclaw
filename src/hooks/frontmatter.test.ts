...
`;
    ...
    expect(parsed.skynet.emoji).toBe("💾");
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.skynet.emoji).toBe("📝");
    expect(parsed.skynet.events).toEqual(["command"]);
    expect(parsed.skynet.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.skynet.install[0].kind).toBe("bundled");
  });
...
describe("resolveSkynetMetadata", () => {
  it("extracts skynet metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveSkynetMetadata(frontmatter);
  ...
  it("returns undefined when skynet key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        skynet: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with Skynet" },
            { id: "npm", kind: "npm", package: "@skynet/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@skynet/hook");
  });
...
`;
    ...
    const skynet = resolveSkynetMetadata(frontmatter);
    expect(skynet).toBeDefined();
    expect(skynet?.emoji).toBe("💾");
    expect(skynet?.events).toEqual(["command:new", "command:reset"]);
    expect(skynet?.requires?.config).toEqual(["workspace.dir"]);
    expect(skynet?.install?.[0].kind).toBe("bundled");
  });
...
`;
    const frontmatter = parseFrontmatter(content);
    const skynet = resolveSkynetMetadata(frontmatter);
    expect(skynet?.emoji).toBe("disk");
    expect(skynet?.events).toEqual(["command:new"]);
  });
});
