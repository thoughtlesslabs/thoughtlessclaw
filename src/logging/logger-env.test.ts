...
const testLogPath = path.join(os.tmpdir(), "skynet-test-env-log-level.log");
const defaultMaxFileBytes = 500 * 1024 * 1024;

describe("SKYNET_LOG_LEVEL", () => {
  let originalEnv: string | undefined;

  beforeEach(() => {
    originalEnv = process.env.SKYNET_LOG_LEVEL;
  ...
  afterEach(() => {
    if (originalEnv === undefined) {
      ...
      process.env.SKYNET_LOG_LEVEL = originalEnv;
    }
    loggingState.invalidEnvLogLevelValue = null;
    resetLogger();
    setLoggerOverride(null);
  ...
  it("warns once and ignores invalid env values", () => {
    ...
    process.env.SKYNET_LOG_LEVEL = "nope";
    ...
    const warnings = stderrSpy.mock.calls
      ...
      .filter((line) => line.includes("SKYNET_LOG_LEVEL"));
    expect(warnings).toHaveLength(1);
    expect(warnings[0]).toContain('Ignoring invalid SKYNET_LOG_LEVEL="nope"');
  });
});
