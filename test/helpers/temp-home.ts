...
function snapshotEnv(): EnvSnapshot {
  return {
    ...
    skynetHome: process.env.SKYNET_HOME,
...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("SKYNET_HOME", snapshot.skynetHome);
  restoreKey("SKYNET_STATE_DIR", snapshot.stateDir);
}
...
function setTempHome(base: string) {
  ...
  // Ensure tests using HOME isolation aren't affected by leaked SKYNET_HOME.
  delete process.env.SKYNET_HOME;
  process.env.SKYNET_STATE_DIR = path.join(base, ".skynet");
...
export async function withTempHome<T>(
  ...
  const base = await fs.mkdtemp(path.join(os.tmpdir(), opts.prefix ?? "skynet-test-home-"));
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".skynet", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
