...
export async function spawnGatewayInstance(name: string): Promise<GatewayInstance> {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `skynet-e2e-${name}-`));
  const configDir = path.join(homeDir, ".skynet");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          SKYNET_SKIP_PROVIDERS: "1",
          SKYNET_SKIP_GMAIL_WATCHER: "1",
          SKYNET_SKIP_CRON: "1",
          SKYNET_SKIP_BROWSER_CONTROL_SERVER: "1",
          SKYNET_SKIP_CANVAS_HOST: "1",
          SKYNET_TEST_MINIMAL_GATEWAY: "1",
          VITEST: "1",
        },
