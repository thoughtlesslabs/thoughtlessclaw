 ...
 * `skynet update`. Use it to verify the completion installation flow
...
\`skynet update\`, \`skynet doctor\`, and \`skynet onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# Skynet Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`skynet completion --install\` later to enable.`));
    return;
  }
