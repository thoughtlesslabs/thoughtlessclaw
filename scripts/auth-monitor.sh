...
#
# Suggested cron: */30 * * * * /home/admin/skynet/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send Skynet notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., skynet-alerts)
...
STATE_FILE="$HOME/.skynet/auth-monitor-state"
...
send_notification() {
    ...
    # Send via Skynet if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use skynet
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via Skynet to $NOTIFY_PHONE..."
            skynet send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! Skynet is down. Run: ssh l36 '~/skynet/scripts/mobile-reauth.sh'" "urgent"
    exit 1
