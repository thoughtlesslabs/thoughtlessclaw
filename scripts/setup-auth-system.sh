#!/bin/bash
# Setup Skynet Auth Management System
...
echo "  2. Subscribe to a topic (e.g., 'skynet-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  Skynet message: Send warning via Skynet itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/skynet-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/skynet-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/skynet-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now skynet-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status skynet-auth-monitor.timer"
echo ""
echo "Quick commands:"
