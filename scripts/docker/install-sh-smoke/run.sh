...
INSTALL_URL="${SKYNET_INSTALL_URL:-https://skynet.bot/install.sh}"
SMOKE_PREVIOUS_VERSION="${SKYNET_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${SKYNET_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="skynet"
PACKAGE_NAME="${SKYNET_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (SKYNET_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${SKYNET_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${SKYNET_INSTALL_LATEST_OUT:-}"
fi
if [[ -n "$CMD_PATH" ]]; then
