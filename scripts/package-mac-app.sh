...
# Build and bundle Skynet into a minimal .app we can open.
# Outputs to dist/Skynet.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/Skynet.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="Skynet"
BUNDLE_ID="${BUNDLE_ID:-ai.skynet.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/skynet/skynet/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/Skynet/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :SkynetBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :SkynetGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/Skynet"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/Skynet"
fi
chmod +x "$APP_ROOT/Contents/MacOS/Skynet"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/Skynet" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/Skynet/Resources/Skynet.icns" "$APP_ROOT/Contents/Resources/Skynet.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/Skynet/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying SkynetKit resources"
SKYNETKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/SkynetKit_SkynetKit.bundle"
if [ -d "$SKYNETKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/SkynetKit_SkynetKit.bundle"
  cp -R "$SKYNETKIT_BUNDLE" "$APP_ROOT/Contents/Resources/SkynetKit_SkynetKit.bundle"
else
  echo "WARN: SkynetKit resource bundle not found at $SKYNETKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running Skynet"
killall -q Skynet 2>/dev/null || true
