#!/usr/bin/env bash
# ClawDock - Docker helpers for Skynet
# Inspired by Simon Willison's "Running Skynet in Docker"
# https://til.simonwillison.net/llms/skynet-docker
#
# Installation:
...
# Common paths to check for Skynet
CLAWDOCK_COMMON_PATHS=(
  "${HOME}/skynet"
  "${HOME}/workspace/skynet"
  "${HOME}/projects/skynet"
  "${HOME}/dev/skynet"
  "${HOME}/code/skynet"
  "${HOME}/src/skynet"
)
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    echo "🦞 Found Skynet at: $found_path"
    echo -n "   Use this location? [Y/n] "
    read -r response
    if [[ "$response" =~ ^[Nn] ]]; then
      echo ""
      echo "Set CLAWDOCK_DIR manually:"
      echo "  export CLAWDOCK_DIR=/path/to/skynet"
      return 1
    fi
    CLAWDOCK_DIR="$found_path"
  else
    echo ""
    echo "❌ Skynet not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
    echo "  git clone https://github.com/skynet/skynet.git ~/skynet"
    echo "  cd ~/skynet && ./docker-setup.sh"
    echo ""
...
_clawdock_read_env_token() {
  ...
  local raw
  raw=$(sed -n 's/^SKYNET_GATEWAY_TOKEN=//p' "${CLAWDOCK_DIR}/.env" | head -n 1)
  if [[ -z "$raw" ]]; then
    return 1
  fi
  _clawdock_trim_quotes "$raw"
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d skynet-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart skynet-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f skynet-gateway
}
...
# Container Access
clawdock-shell() {
  _clawdock_compose exec skynet-gateway \
    bash -c 'echo "alias skynet=\"./skynet.mjs\"" > /tmp/.bashrc_skynet && bash --rcfile /tmp/.bashrc_skynet'
}
...
clawdock-cli() {
  _clawdock_compose run --rm skynet-cli "$@"
}

# Maintenance
clawdock-rebuild() {
  _clawdock_compose build skynet-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "SKYNET_GATEWAY_TOKEN=$token" skynet-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" skynet-gateway \
    bash -c './skynet.mjs config set gateway.remote.token "$TOKEN" && ./skynet.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec skynet-gateway \
    bash -c "./skynet.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart skynet-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm skynet-cli dashboard --no-open 2>&1)
  exit_status=$?
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec skynet-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
  ...
  if [ $exit_status -ne 0 ]; then
    ...
    echo -e "      $(_cmd 'skynet config get gateway.remote.token')"
    return 1
  fi
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}🦞 ClawDock - Docker Helpers for Skynet${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (skynet alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to skynet project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.skynet)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-shell)"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'skynet channels login --channel whatsapp')"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'skynet status')"
  echo ""
  ...
  echo -e "${_CLR_BLUE}📚 Docs: ${_CLR_RESET}${_CLR_CYAN}https://docs.skynet.ai${_CLR_RESET}"
  echo ""
}
