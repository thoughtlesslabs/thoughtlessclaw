...
BASE_IMAGE="${BASE_IMAGE:-skynet-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-skynet-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=skynet.sandbox=1)
NOTE
