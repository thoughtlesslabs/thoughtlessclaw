...
IMAGE_NAME="skynet-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="skynet-net-e2e-$$"
GW_NAME="skynet-gateway-e2e-$$"
...
echo "Starting gateway container..."
	docker run --rm -d \
	  --name "$GW_NAME" \
	  --network "$NET_NAME" \
	  -e "SKYNET_GATEWAY_TOKEN=$TOKEN" \
	  -e "SKYNET_SKIP_CHANNELS=1" \
	  -e "SKYNET_SKIP_GMAIL_WATCHER=1" \
	  -e "SKYNET_SKIP_CRON=1" \
	  -e "SKYNET_SKIP_CANVAS_HOST=1" \
	  "$IMAGE_NAME" \
