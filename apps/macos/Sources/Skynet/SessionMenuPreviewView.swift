...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.skynet", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> SkynetSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: SkynetSessionPreviewEntry,
    ...
    private static func cache(payload: SkynetSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: SkynetChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [SkynetKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [SkynetKit.AnyCodable]) -> [SkynetChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(SkynetChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: SkynetChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: SkynetChatMessage) -> Bool {
    ...
    private static func toolNames(for message: SkynetChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: SkynetChatMessage) -> String? {
