...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.skynet/skynet.json. Prefer SKYNET_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.skynet/skynet.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.skynet/skynet.json. Restart the gateway to apply."
        }
