...
actor TalkModeRuntime {
    ...
    private let logger = Logger(subsystem: "ai.skynet", category: "talk.runtime")
    private let ttsLogger = Logger(subsystem: "ai.skynet", category: "talk.tts")
    ...
    private func latestAssistantText(sessionKey: String, since: Double? = nil) async -> String? {
        do {
            ...
            let decoded: [SkynetChatMessage] = messages.compactMap { item in
