...
final class PeekabooBridgeHostCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.skynet", category: "PeekabooBridge")

    private var host: PeekabooBridgeHost?
    private var services: SkynetPeekabooBridgeServices?
    private static var skynetSocketPath: String {
        let fileManager = FileManager.default
        let base = fileManager.urls(for: .applicationSupportDirectory, in: .userDomainMask).first
            ?? fileManager.homeDirectoryForCurrentUser.appendingPathComponent("Library/Application Support")
        let directory = base.appendingPathComponent("Skynet", isDirectory: true)
    ...
    private func startIfNeeded() async {
        ...
        let services = SkynetPeekabooBridgeServices()
        ...
        let host = PeekabooBridgeHost(
            socketPath: Self.skynetSocketPath,
            server: server,
        ...
        await host.start()
        self.logger
            .info("PeekabooBridge host started at \(Self.skynetSocketPath, privacy: .public)")
    }
...
@MainActor
private final class SkynetPeekabooBridgeServices: PeekabooBridgeServiceProviding {
    ...
    init() {
        let logging = LoggingService(subsystem: "ai.skynet.peekaboo")
