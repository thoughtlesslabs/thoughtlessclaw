...
enum CommandResolver {
    private static let projectRootDefaultsKey = "skynet.gatewayProjectRootPath"
    private static let helperName = "skynet"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let skynetEntry = root.appendingPathComponent("skynet.mjs").path
        if FileManager().isReadableFile(atPath: skynetEntry) { return skynetEntry }
        let binEntry = root.appendingPathComponent("bin/skynet.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/skynet")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let skynetPaths = self.skynetManagedPaths(home: home)
        if !skynetPaths.isEmpty {
            extras.insert(contentsOf: skynetPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + skynetPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func skynetManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".skynet"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func skynetExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectSkynetExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("skynet.mjs").path,
            root.appendingPathComponent("bin/skynet.js").path,
        ]
    ...
    static func hasAnySkynetInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.skynetExecutable(searchPaths: searchPaths) != nil { return true }
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        case let .success(runtime):
            let root = self.projectRoot()
            if let skynetPath = self.projectSkynetExecutable(projectRoot: root) {
                return [skynetPath, subcommand] + extraArgs
            }
            ...
            if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
                ...
                return [pnpm, "--silent", "skynet", subcommand] + extraArgs
            }
            if let skynetPath = self.skynetExecutable(searchPaths: searchPaths) {
                return [skynetPath, subcommand] + extraArgs
            }

            let missingEntry = """
            skynet entrypoint missing (looked for dist/index.js or skynet.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real skynet CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/skynet"
        ...
        \(cliSection)
        \(projectSection)
        if command -v skynet >/dev/null 2>&1; then
          CLI="$(command -v skynet)"
          skynet \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/skynet.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/skynet.mjs"
            node "$PRJ/skynet.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/skynet.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/skynet.js"
            node "$PRJ/bin/skynet.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent skynet"
          pnpm --silent skynet \(quotedArgs);
        else
          echo "skynet CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? SkynetConfigFile.loadDict()
