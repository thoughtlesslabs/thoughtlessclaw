...
enum DebugActions {
    private static let verboseDefaultsKey = "skynet.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "skynet.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = SkynetPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "Skynet", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = SkynetPaths.configURL
        guard
