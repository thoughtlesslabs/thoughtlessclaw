...
struct AnthropicAuthControls: View {
    ...
    @State private var oauthStatus: SkynetOAuthStore.AnthropicOAuthStatus = SkynetOAuthStore.anthropicOAuthStatus()
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack(spacing: 10) {
                ...
                .disabled(!FileManager().fileExists(atPath: SkynetOAuthStore.oauthURL().path))
    ...
    private func refresh() {
        let imported = SkynetOAuthStore.importLegacyAnthropicOAuthIfNeeded()
        self.oauthStatus = SkynetOAuthStore.anthropicOAuthStatus()
        if imported != nil {
    ...
    private func finishOAuth() async {
        ...
        do {
            ...
            try SkynetOAuthStore.saveAnthropicOAuth(creds)
            self.refresh()
            self.pkce = nil
            self.code = ""
            self.statusText = "Connected. Skynet can now use Claude via OAuth."
        } catch {
...
extension AnthropicAuthControls {
    init(
        ...
        oauthStatus: SkynetOAuthStore.AnthropicOAuthStatus,
