...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["SKYNET_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        return URL(fileURLWithPath: "/tmp/skynet")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("skynet-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("skynet-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/skynet/ (rolling or stdout), or nil if none exist.
