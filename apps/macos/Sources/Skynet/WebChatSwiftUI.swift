...
private let webChatSwiftLogger = Logger(subsystem: "ai.skynet", category: "WebChatSwiftUI")
...
struct MacGatewayChatTransport: SkynetChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> SkynetChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        return try JSONDecoder().decode(SkynetChatSessionsListResponse.self, from: data)
    }
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> SkynetChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                SkynetGatewayHealthOK.self,
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<SkynetChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any SkynetChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = SkynetChatViewModel(sessionKey: sessionKey, transport: transport)
        let accent = Self.color(fromHex: AppStateStore.shared.seamColorHex)
        self.hosting = NSHostingController(rootView: SkynetChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<SkynetChatView>) -> NSViewController
    {
