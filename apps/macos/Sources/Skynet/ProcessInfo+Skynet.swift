...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.skynet.mac`) even if the shipped
    ...
    {
        if environment["SKYNET_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "skynet.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "skynet.nixMode") { return true }
