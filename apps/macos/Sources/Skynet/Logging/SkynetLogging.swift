...
enum SkynetLogging {
    ...
    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            ...
            let osHandler = SkynetOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = SkynetFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        SkynetLogging.bootstrapIfNeeded()
        let label = SkynetLogging.makeLabel(subsystem: subsystem, category: category)
...
struct SkynetOSLogHandler: LogHandler {
...
struct SkynetFileLogHandler: LogHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = SkynetLogging.parseLabel(self.label)
