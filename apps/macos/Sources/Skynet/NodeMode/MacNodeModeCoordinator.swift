...
final class MacNodeModeCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.skynet", category: "mac-node")
                    ...
                    onInvoke: { [weak self] req in
                        guard let self else {
                            return BridgeInvokeResponse(
                                id: req.id,
                                ok: false,
                                error: SkynetNodeError(code: .unavailable, message: "UNAVAILABLE: node not ready"))
                        }
    ...
    private func currentCaps() -> [String] {
        var caps: [String] = [SkynetCapability.canvas.rawValue, SkynetCapability.screen.rawValue]
        if UserDefaults.standard.object(forKey: cameraEnabledKey) as? Bool ?? false {
            caps.append(SkynetCapability.camera.rawValue)
        }
        let rawLocationMode = UserDefaults.standard.string(forKey: locationModeKey) ?? "off"
        if SkynetLocationMode(rawValue: rawLocationMode) != .off {
            caps.append(SkynetCapability.location.rawValue)
        }
        return caps
    }
    ...
    private func currentCommands(caps: [String]) -> [String] {
        var commands: [String] = [
            ...
            SkynetSystemCommand.which.rawValue,
            SkynetSystemCommand.run.rawValue,
            SkynetSystemCommand.execApprovalsGet.rawValue,
            SkynetSystemCommand.execApprovalsSet.rawValue,
        ]

        let capsSet = Set(caps)
        if capsSet.contains(SkynetCapability.camera.rawValue) {
        ...
        if capsSet.contains(SkynetCapability.location.rawValue) {
