...
struct ControlAgentEvent: Codable, Sendable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: SkynetProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.skynet", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: SkynetKit.AnyCodable]()) {
                $0[$1.key] = SkynetKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: SkynetProtocol.AnyCodable?) -> [String: SkynetProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: SkynetProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: SkynetKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: SkynetProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("skynet.control.heartbeat")
    static let controlAgentEvent = Notification.Name("skynet.control.agent")
}
