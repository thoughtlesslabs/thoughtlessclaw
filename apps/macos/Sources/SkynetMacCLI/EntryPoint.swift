...
struct SkynetMacCLI {
    static func main() async {
        ...
        default:
            fputs("skynet-mac: unknown command\n", stderr)
            printUsage()
            exit(1)
        }
    }
...
private func printUsage() {
    ...
    Usage:
      skynet-mac connect [--url <ws://host:port>] [--token <token>] [--password <password>]
                           [--mode <local|remote>] [--timeout <ms>] [--probe] [--json]
                           [--client-id <id>] [--client-mode <mode>] [--display-name <name>]
                           [--role <role>] [--scopes <a,b,c>]
      skynet-mac discover [--timeout <ms>] [--json] [--include-local]
      skynet-mac wizard [--url <ws://host:port>] [--token <token>] [--password <password>]
                          [--mode <local|remote>] [--workspace <path>] [--json]

    Examples:
      skynet-mac connect
      skynet-mac connect --url ws://127.0.0.1:18789 --json
      skynet-mac discover --timeout 3000 --json
      skynet-mac wizard --mode local
    """)
}
