...
@Suite struct InstancesStoreTests {
    ...
    func presenceEventPayloadDecodesViaJSONEncoder() {
        ...
        let entry: [String: SkynetProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: SkynetProtocol.AnyCodable] = [
            "presence": .init([SkynetProtocol.AnyCodable(entry)]),
        ]
        let payload = SkynetProtocol.AnyCodable(payloadMap)
