...
final class ShareViewController: UIViewController {
    ...
    private let logger = Logger(subsystem: "ai.skynet.ios", category: "ShareExtension")
    ...
    private func setupUI() {
        ...
        self.sendButton.setTitle("Send to Skynet", for: .normal)
    ...
    private func sendCurrentDraft() async {
        ...
        self.showStatus("Sending to Skynet gateway…")
        ...
        do {
            ...
            self.showStatus("Sent to Skynet.")
    ...
    private func sendMessageToGateway(_ message: String, attachments: [ShareAttachment]) async throws {
        guard let config = ShareGatewayRelaySettings.loadConfig() else {
            throw NSError(
                domain: "SkynetShare",
                code: 10,
                userInfo: [NSLocalizedDescriptionKey: "Skynet is not connected to a gateway yet."])
        }
        ...
        let makeOptions: (String) -> GatewayConnectOptions = { clientId in
            GatewayConnectOptions(
                ...
                clientId: clientId,
                clientMode: "node",
                clientDisplayName: "Skynet Share",
        ...
        do {
            try await gateway.connect(
                ...
                connectOptions: makeOptions("skynet-ios"),
                sessionBox: nil,
                onConnected: {},
                onDisconnected: { _ in },
                onInvoke: { req in
