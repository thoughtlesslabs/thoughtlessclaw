...
final class GatewayConnectionController {
    ...
    private func currentCaps() -> [String] {
        var caps = [SkynetCapability.canvas.rawValue, SkynetCapability.screen.rawValue]
        ...
        if cameraEnabled { caps.append(SkynetCapability.camera.rawValue) }

        let voiceWakeEnabled = UserDefaults.standard.bool(forKey: VoiceWakePreferences.enabledKey)
        if voiceWakeEnabled { caps.append(SkynetCapability.voiceWake.rawValue) }

        let locationModeRaw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        let locationMode = SkynetLocationMode(rawValue: locationModeRaw) ?? .off
        if locationMode != .off { caps.append(SkynetCapability.location.rawValue) }

        caps.append(SkynetCapability.device.rawValue)
        if WatchMessagingService.isSupportedOnDevice() {
            caps.append(SkynetCapability.watch.rawValue)
        }
        caps.append(SkynetCapability.photos.rawValue)
        caps.append(SkynetCapability.contacts.rawValue)
        caps.append(SkynetCapability.calendar.rawValue)
        caps.append(SkynetCapability.reminders.rawValue)
        if Self.motionAvailable() {
            caps.append(SkynetCapability.motion.rawValue)
        }
    ...
    private func currentCommands() -> [String] {
        var commands: [String] = [
            ...
            SkynetScreenCommand.record.rawValue,
            SkynetSystemCommand.notify.rawValue,
            SkynetChatCommand.push.rawValue,
        ...
        if caps.contains(SkynetCapability.camera.rawValue) {
            commands.append(SkynetCameraCommand.list.rawValue)
            commands.append(SkynetCameraCommand.snap.rawValue)
            commands.append(SkynetCameraCommand.clip.rawValue)
        }
        if caps.contains(SkynetCapability.location.rawValue) {
            commands.append(SkynetLocationCommand.get.rawValue)
        }
        if caps.contains(SkynetCapability.device.rawValue) {
            commands.append(SkynetDeviceCommand.status.rawValue)
            commands.append(SkynetDeviceCommand.info.rawValue)
        }
        if caps.contains(SkynetCapability.watch.rawValue) {
            commands.append(SkynetWatchCommand.status.rawValue)
            commands.append(SkynetWatchCommand.notify.rawValue)
        }
        if caps.contains(SkynetCapability.photos.rawValue) {
            commands.append(SkynetPhotosCommand.latest.rawValue)
        }
        if caps.contains(SkynetCapability.contacts.rawValue) {
            commands.append(SkynetContactsCommand.search.rawValue)
            commands.append(SkynetContactsCommand.add.rawValue)
        }
        if caps.contains(SkynetCapability.calendar.rawValue) {
            commands.append(SkynetCalendarCommand.events.rawValue)
            commands.append(SkynetCalendarCommand.add.rawValue)
        }
        if caps.contains(SkynetCapability.reminders.rawValue) {
            commands.append(SkynetRemindersCommand.list.rawValue)
            commands.append(SkynetRemindersCommand.add.rawValue)
        }
        if caps.contains(SkynetCapability.motion.rawValue) {
            commands.append(SkynetMotionCommand.activity.rawValue)
            commands.append(SkynetMotionCommand.pedometer.rawValue)
        }
