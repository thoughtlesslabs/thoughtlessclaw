...
final class ContactsService: ContactsServicing {
    ...
    func search(params: SkynetContactsSearchParams) async throws -> SkynetContactsSearchPayload {
        ...
        return SkynetContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: SkynetContactsAddParams) async throws -> SkynetContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return SkynetContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return SkynetContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> SkynetContactPayload {
        SkynetContactPayload(
