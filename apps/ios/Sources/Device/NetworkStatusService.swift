...
final class NetworkStatusService: @unchecked Sendable {
    func currentStatus(timeoutMs: Int = 1500) async -> SkynetNetworkStatusPayload {
    ...
    private static func payload(from path: NWPath) -> SkynetNetworkStatusPayload {
        let status: SkynetNetworkPathStatus = switch path.status {
    ...
    private static func fallbackPayload() -> SkynetNetworkStatusPayload {
        SkynetNetworkStatusPayload(
            status: .unsatisfied,
            isExpensive: false,
            isConstrained: false,
            interfaces: [.other])
