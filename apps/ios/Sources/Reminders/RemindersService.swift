...
final class RemindersService: RemindersServicing {
    func list(params: SkynetRemindersListParams) async throws -> SkynetRemindersListPayload {
        ...
        let payload = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<[SkynetReminderPayload], Error>) in
        ...
        return SkynetRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: SkynetRemindersAddParams) async throws -> SkynetRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = SkynetReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return SkynetRemindersAddPayload(reminder: payload)
    }
