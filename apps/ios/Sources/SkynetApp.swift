...
@MainActor
final class SkynetAppDelegate: NSObject, UIApplicationDelegate, @preconcurrency UNUserNotificationCenterDelegate {
    private let logger = Logger(subsystem: "ai.skynet.ios", category: "Push")
    private let backgroundWakeLogger = Logger(subsystem: "ai.skynet.ios", category: "BackgroundWake")
    private static let wakeRefreshTaskIdentifier = "ai.skynet.ios.bgrefresh"
...
enum WatchPromptNotificationBridge {
    static let typeKey = "skynet.type"
    static let typeValue = "watch.prompt"
    static let promptIDKey = "skynet.watch.promptId"
    static let sessionKeyKey = "skynet.watch.sessionKey"
    static let actionPrimaryIDKey = "skynet.watch.action.primary.id"
    static let actionPrimaryLabelKey = "skynet.watch.action.primary.label"
    static let actionSecondaryIDKey = "skynet.watch.action.secondary.id"
    static let actionSecondaryLabelKey = "skynet.watch.action.secondary.label"
    static let actionPrimaryIdentifier = "skynet.watch.action.primary"
    static let actionSecondaryIdentifier = "skynet.watch.action.secondary"
    static let actionIdentifierPrefix = "skynet.watch.action."
    static let actionIDKeyPrefix = "skynet.watch.action.id."
    static let actionLabelKeyPrefix = "skynet.watch.action.label."
    static let categoryPrefix = "skynet.watch.prompt.category."

    @MainActor
    static func scheduleMirroredWatchPromptNotificationIfNeeded(
        invokeID: String,
        params: SkynetWatchNotifyParams,
        sendResult: WatchNotificationSendResult) async
    {
        ...
        let normalizedActions = (params.actions ?? []).compactMap { action -> SkynetWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return SkynetWatchAction(id: id, label: label, style: action.style)
        }
        ...
        content.title = title.isEmpty ? "Skynet" : title
        content.body = body
    ...
    private static func categoryActions(_ actions: [SkynetWatchAction]) -> [UNNotificationAction] {
...
extension SkynetApp {
    private static func installUncaughtExceptionLogger() {
        NSLog("Skynet: installing uncaught exception handler")
