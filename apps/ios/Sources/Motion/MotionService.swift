...
final class MotionService: MotionServicing {
    func activities(params: SkynetMotionActivityParams) async throws -> SkynetMotionActivityPayload {
        ...
        let mapped = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<[SkynetMotionActivityEntry], Error>) in
        ...
        return SkynetMotionActivityPayload(activities: mapped)
    }
    ...
    func pedometer(params: SkynetPedometerParams) async throws -> SkynetPedometerPayload {
        ...
        let payload = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<SkynetPedometerPayload, Error>) in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    cont.resume(throwing: error)
                } else {
                    let formatter = ISO8601DateFormatter()
                    let payload = SkynetPedometerPayload(
