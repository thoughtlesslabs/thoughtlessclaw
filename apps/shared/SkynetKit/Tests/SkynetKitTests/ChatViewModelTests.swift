...
private final class TestChatTransport: @unchecked Sendable, SkynetChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [SkynetChatHistoryPayload]
    private let sessionsResponses: [SkynetChatSessionsListResponse]

    private let stream: AsyncStream<SkynetChatTransportEvent>
    private let continuation: AsyncStream<SkynetChatTransportEvent>.Continuation

    init(
        historyResponses: [SkynetChatHistoryPayload],
        sessionsResponses: [SkynetChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<SkynetChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> SkynetChatHistoryPayload {
        ...
        return self.historyResponses.last ?? SkynetChatHistoryPayload(
        ...
        attachments _: [SkynetChatAttachmentPayload]) async throws -> SkynetChatSendResponse
    {
        ...
        return SkynetChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> SkynetChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? SkynetChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: SkynetChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    @Test func streamsAssistantAndClearsOnFinal() async throws {
        ...
        let history1 = SkynetChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
        let history2 = SkynetChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [
                AnyCodable([
        ...
        let vm = await MainActor.run { SkynetChatViewModel(sessionKey: "main", transport: transport) }
    ...
    @Test func clearsStreamingOnExternalFinalEvent() async throws {
        let sessionId = "sess-main"
        let history = SkynetChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let history = SkynetChatHistoryPayload(
            ...
            messages: [],
            thinkingLevel: "off")
        let sessions = SkynetChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesIncludeCurrentWhenMissing() async throws {
        ...
        let transport = TestChatTransport(
            historyResponses: [history],
            sessionsResponses: [sessions])
        let vm = await MainActor.run { SkynetChatViewModel(sessionKey: "custom", transport: transport) }
        await MainActor.run { vm.load() }
        try await waitUntil("sessions loaded") { await MainActor.run { !vm.sessions.isEmpty } }
