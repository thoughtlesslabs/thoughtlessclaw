...
@MainActor
struct SkynetChatComposer: View {
    @Bindable var viewModel: SkynetChatViewModel
        ...
        .background {
            ...
            if self.style == .standard {
                let shape = UnevenRoundedRectangle(
                    ...
                    style: .continuous)
                shape
                    .fill(SkynetChatTheme.composerBackground)
                    .overlay(shape.strokeBorder(SkynetChatTheme.composerBorder, lineWidth: 1))
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(
                    ...
                    id: \SkynetPendingAttachment.id)
                { (att: SkynetPendingAttachment) in
                    HStack(spacing: 6) {
                        if let img = att.preview {
                            SkynetPlatformImageFactory.image(img)
                                .resizable()
                                .scaledToFill()
    ...
    private var editor: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(SkynetChatTheme.composerField)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
                        .strokeBorder(SkynetChatTheme.composerBorder)))
    ...
    private var connectionPill: some View {
        ...
        .padding(.horizontal, 8)
        .padding(.vertical, 4)
        .background(SkynetChatTheme.subtleCard)
        .clipShape(Capsule())
    }
