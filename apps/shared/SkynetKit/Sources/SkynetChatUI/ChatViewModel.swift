...
private let chatUILogger = Logger(subsystem: "ai.skynet", category: "SkynetChatUI")

@MainActor
@Observable
public final class SkynetChatViewModel {
    public private(set) var messages: [SkynetChatMessage] = []
    ...
    public var attachments: [SkynetPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [SkynetChatPendingToolCall] = []
    public private(set) var sessions: [SkynetChatSessionEntry] = []
    private let transport: any SkynetChatTransport
    ...
    private var pendingToolCallsById: [String: SkynetChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(sessionKey: String, transport: any SkynetChatTransport) {
    ...
    public var sessionChoices: [SkynetChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }

        var result: [SkynetChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: SkynetPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [SkynetChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: SkynetChatMessage.self))
                .map { Self.stripInboundMetadata(from: $0) }
        }
        return Self.dedupeMessages(decoded)
    }

    private static func stripInboundMetadata(from message: SkynetChatMessage) -> SkynetChatMessage {
        guard message.role.lowercased() == "user" else {
            return message
        }

        let sanitizedContent = message.content.map { content -> SkynetChatMessageContent in
            guard let text = content.text else { return content }
            let cleaned = ChatMarkdownPreprocessor.preprocess(markdown: text).cleaned
            return SkynetChatMessageContent(
                type: content.type,
                text: cleaned,
    ...
    private static func messageIdentityKey(for message: SkynetChatMessage) -> String? {
    ...
    private static func reconcileMessageIDs(
        previous: [SkynetChatMessage],
        incoming: [SkynetChatMessage]) -> [SkynetChatMessage]
    {
    ...
    private static func dedupeMessages(_ messages: [SkynetChatMessage]) -> [SkynetChatMessage] {
        var result: [SkynetChatMessage] = []
    ...
    private static func dedupeKey(for message: SkynetChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [SkynetChatMessageContent] = [
            SkynetChatMessageContent(
                type: "text",
                text: messageText,
                thinking: nil,
        ...
        let encodedAttachments = self.attachments.map { att -> SkynetChatAttachmentPayload in
            SkynetChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> SkynetChatSessionEntry {
        SkynetChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func handleTransportEvent(_ evt: SkynetChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: SkynetChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: SkynetAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = SkynetChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> SkynetPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
